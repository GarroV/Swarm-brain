...
#   · ОДИН PR за ПРОГОН (не за ночь). Прогонов за ночь может быть несколько — решение владельца
#     16.09.2026: «за ночь хоть миллион прогонов, похер» (docs/decisions/2026-09-16-runs-per-night.md).
#     Ограничение осталось ровно одно: в одном прогоне не смешиваем два PR, чтобы при поломке
#     было понятно, чья она.
...
# Во сколько сегодня ночью проходят прогоны — по Белграду, а не в UTC. Кронов два (21:00 и 22:00
# UTC): летом это 23:00 и 00:00, то есть оба внутри окна; зимой — 22:00 и 23:00, и первый выходит
# вне окна. Печатаем ФАКТ, потому что сообщение «дождутся следующей ночи» было неверным и на нём
# уже построили ошибочный график раскаток (issue #157). Смещение берём из текущего времени, чтобы
# не зашивать дату перехода на зимнее время.
night_slots() {
  local off h out=""
  off=$(( 10#$(TZ=Europe/Belgrade date +%H) - 10#$(date -u +%H) ))
  [ "$off" -lt -12 ] && off=$((off + 24)); [ "$off" -gt 12 ] && off=$((off - 24))
  for utc in 21 22; do
    h=$(( (utc + off + 24) % 24 ))
    { [ "$h" -ge 23 ] || [ "$h" -lt 6 ]; } && out="$out $(printf '%02d:00' "$h")"
  done
  echo "${out# }"
}
...
REMAINING=$((COUNT - 1))
[ "$REMAINING" -gt 0 ] && say "Ещё помечено: $REMAINING — по одному за прогон. Прогоны этой ночью по Белграду: $(night_slots); ближайший возьмёт следующий по очереди."
