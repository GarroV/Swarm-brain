...
тиках подряд; (г) для встреч из **Google Calendar** — **наступил плановый конец** (`endISO` из `meeting-current`)
и системная дорожка молчит ~30с → стоп «встреча завершилась (по календарю)»; переработку не рубим, но с
потолком `end+30мин` (как у Granola — плановый конец сильный сигнал; A→B решается сам, у каждого события свой конец).
