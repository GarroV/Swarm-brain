...
## Что дальше (итерации)
1. ~~Идентичность по календарю~~ ✅; ~~авто-старт с согласием~~ ✅; ~~room-id из URL (Meet/Контур)~~ ✅.
2. Резка/сжатие длинных записей под лимит OpenAI 25 МБ на дорожку.
3. Меню-бар: онбординг ввода токена, индикатор статуса, «переотправить».
4. Очередь/ретраи на диск (переживать перезапуск), как в `SwarmClient` заложено.
