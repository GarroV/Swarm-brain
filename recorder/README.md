...
→ manual**. Эта лестница работает при ЛЮБОМ способе старта, включая «Записать» из меню и ответ на
подсказку о звонке (`manualStartIdentity`, решение владельца 18.09.2026 «календарь надо брать всегда»,
[канон](../docs/decisions/2026-09-18-calendar-always-on-manual-start.md)): до этого календарь попадал в
запись только из календарного предложения в капсуле, и живая встреча писалась как `room`/`manual` мимо
склейки с коллегами. Календарь берём, пока ответ сервера свежий (≤90 с) и слот идёт или начнётся в
пределах 5 минут (`RecorderKit/StartIdentity`). Запись включили на ОФЛАЙН встрече, а в календаре стоит
другое событие → пункт меню **«Это не „<название>“ — писать отдельно»** снимает привязку, запись уедет
как `manual` (новая встреча, чужую не тронет). Требует, чтобы пользователь подключил Google Calendar в вебе (Настройки → Google-календарь).
...
**Правила (а) и (б) работают только после начала разговора** (`RecorderKit/ConversationGate`, issue #379): разговор начался, когда собеседников слышно на системной дорожке 2 тика подряд (≈10 с; одиночный звук входа — «блип», не в счёт). До этого (а) и (б) ловят не конец звонка, а ожидание: человек нажал «Записать» за 5 минут до встречи, сидит в лобби один — и запись обрывалась через 3 мин тишины, а сама встреча шла уже без записи (17.09.2026: 6,5 мин тишины, стоп секунда в секунду через 180 с после последнего звука). Пока разговора не было, запись держат только (в), бэкстоп «собеседников не слышно 15 мин», лимиты и сон. Состояние ворот видно в dbg-строке тика: `talk=true/false`.
