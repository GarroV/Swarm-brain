...
// Отдаёт функция meeting-context: тезисы ПОСЛЕДНЕЙ встречи с этой стороной (страной)
// и задачи, привязанные к той встрече, с добивкой по стране. Считает сервер — панель
...
struct MeetingContext: Codable {
    ...
    struct Task: Codable {
        ...
        let status: String
        /// "meeting" — задача той самой прошлой встречи; "country" — добивка по стране.
        let source: String
    }
    let country: String?
