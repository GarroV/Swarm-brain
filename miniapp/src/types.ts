...
export type Entry = {
  ...
  // сказать, что именно произошло, а не «опубликовано».
  duplicate?: boolean;
  // true — версия публикующего оказалась ПОЛНЕЕ и стала содержимым общей записи
  // («в базу по дефолту идёт самая полная встреча», issue #176). false — в базе осталась
  // прежняя версия; причина в arbitration.
  replaced?: boolean;
  arbitration?: "incoming_richer" | "incoming_has_transcript" | "published_edited" | "incoming_edited" | "not_richer";
...
export type AgentMeeting = {
  ...
  // Эта же встреча уже лежит в базе (её опубликовал другой участник) — приходит в ДЕТАЛИ
