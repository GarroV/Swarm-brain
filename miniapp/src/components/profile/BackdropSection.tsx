...
import { applyBackdrop, BACKDROP_OPTIONS, type BackdropId, notifyCustomBackdropChanged } from "@/lib/backdrop";
import { type CustomBackdrop, loadCustomBackdrop } from "@/lib/backdropStore";
import { CustomBackdropPanel } from "./CustomBackdropPanel";
...
function Preview({ id, customUrl }: { id: BackdropId; customUrl: string | null }) {
  const base = "h-14 w-full rounded-[7px] border border-line-2";
  if (id === "custom") {
    return customUrl
      ? <div className={`${base} bg-cover bg-center`} style={{ backgroundImage: `url("${customUrl}")` }} />
      : <div className={`${base} grid place-items-center border-dashed bg-surface-2 text-ink-mute`}><RoyIcon name="plus" size={18} /></div>;
  }
...
export function BackdropSection() {
  ...
  // Панель своей картинки открыта: выбран вариант «custom» или человек нажал плитку без картинки.
  const [panelOpen, setPanelOpen] = useState(false);

  useEffect(() => { void loadCustomBackdrop().then(setCustom); }, []);
  useEffect(() => {
    if (!custom) { setCustomUrl(null); return; }
    const u = URL.createObjectURL(custom.blob);
    setCustomUrl(u);
    return () => URL.revokeObjectURL(u);
  }, [custom]);

  const choose = (id: BackdropId) => {
    if (id === "custom") setPanelOpen(true);
    if (id === "custom" && !custom) return; // сначала картинка — выбрать пустой вариант нельзя
    select(id);
  };

  // Применить и сохранить в профиль — без проверок выше (их делает choose).
  const select = (id: BackdropId) => {
  ...
  return (
    <div className="space-y-2.5">
      <div role="radiogroup" aria-label={dt("Фон", "Background")} className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
        {BACKDROP_OPTIONS.map((o) => {
          ...
          return (
            ...
            >
              <Preview id={o.id} customUrl={customUrl} />
      ...
      {(panelOpen || current === "custom") && (
        <CustomBackdropPanel
          rec={custom}
          onSaved={(rec) => {
            setCustom(rec);
            notifyCustomBackdropChanged();
            select("custom");
          }}
          onDeleted={() => {
            setCustom(null);
            notifyCustomBackdropChanged();
            if (current === "custom") select("galaxy");
          }}
        />
      )}
      {saveFailed && (
