...
function RecorderSection() {
  ...
  const getCommand = async (reissue: boolean) => {
    // Про перекрытие говорим, только если сервер уже умеет его давать. Признак — updateOneLiner
    // в ответе: он появляется той же раскаткой, что и перекрытие. Иначе веб (он уезжает раньше,
    // Pages собирает прод сам на мёрж) пообещал бы сутки запаса, которых на сервере ещё нет.
    const serverHasGrace = !!setup?.updateOneLiner;
    if (reissue && !(await confirm({
      title: "Перевыпустить токен bumblebee?",
      description: serverHasGrace
        ? "Нужно, только если ты потерял токен или ставишь bumblebee на ДРУГОЙ мак. Для обновления на этом маке токен менять не надо — есть команда выше. Прежний токен после перевыпуска поработает ещё сутки, чтобы записи не потерялись."
        : "Старый токен перестанет работать — bumblebee нужно будет переустановить командой ниже.",
