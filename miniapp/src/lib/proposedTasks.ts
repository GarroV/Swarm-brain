...
/**
 * Кому РЕАЛЬНО уйдёт задача при публикации из разбора: имя от GPT → id, а НЕ подтянулось по
 * любой причине — тому, кто эту задачу сейчас генерирует.
 *
 * Прод-инцидент 28.08.2026: коллега разобрала встречу и нажала «Добавить». Ответственных модель
 * не назвала, задачи легли без исполнителя и без срока — то есть мимо «Сегодня» (список требует
 * срок), мимо линзы «Мои» (задача ни на ком) и мимо секции с её именем в группировке по
 * сотрудникам. Тост сказал «Добавлено 2 задачи», человек их нигде не нашёл и решил, что продукт
 * ничего не сохранил.
 *
 * Решение владельца в тот же день (docs/decisions/2026-08-28-assignee-falls-back-to-author.md):
 * «не смогли подтянуть исполнителя — ставим того, кто генерировал задачу». ЛЮБОЙ промах —
 * имя не названо, названо чужое («Илья Голдин» не из команды), названо неоднозначно (два Петра) —
 * значит задача того, кто её забирает. Ничьих задач из разбора больше не рождается: перевесить
 * на себя лишнюю задачу видно и правится одним кликом, а пропажу не видит никто.
 *
 * Единственный случай null — личность разбирающего неизвестна (`/me` не ответил): выдумывать
 * некого, и тост тогда честно пишет «· N без исполнителя».
 */
export function effectiveAssigneeId(
  ...
  return resolveAssigneeId(assignee, users) ?? meId;
}
