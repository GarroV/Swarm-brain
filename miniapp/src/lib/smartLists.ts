...
//   • `pending` — статус, который писали бот и read-ai-webhook при авто-извлечении задач из
//     встречи; подтвердить такую задачу можно было только в Telegram (issue #208). Мьют —
//     решение владельца 03.09.2026, поэтому он ОСТАЁТСЯ. Но 05.09.2026 убраны сами трубы,
//     которые этот статус писали: задачи из встречи теперь делает человек кнопкой
//     «Сгенерировать задачи» в вебе, и в базу они едут сразу активными. Мьют стал вторым
//     слоем: первый — детектор `_shared/tasks/no-pending.test.ts`, который не даст завести
//     новый источник pending.
// Замьюченное не показывается НИГДЕ в модуле задач: ни в списках, ни в счётчиках, ни в метках.
const MUTED_STATUSES: ReadonlySet<string> = new Set(["pending", "backlog"]);
