...
// Линт просит адрес с расширением. Здесь — нельзя: веб пишет `@/types` без него (норма для
// Next), а `@/types.ts` уже не соберёт Next. Правило снято точечно и только на этот импорт
// типов; рантайм-зависимостей у модуля нет.
