...
| T070 | orchestrator | T045,T060,T149 | done | `startForMeeting` / `stop` через dockerode; ручной запуск по ссылке работает | #352 | 1 |
...
| T153 | orchestrator | T151 | todo | Перехват записи рекордером (арбитраж `meeting-claim`) гасит `meetings.agent_last_recording` тем же обновлением (D019); тест ядра: после перехвата сторож не шлёт алерт новому `claim_owner`; решить, что бот делает на 403 после перехвата (сейчас `meeting-heartbeat` при отказе по встрече не освежает и `service_agents.last_seen_at` — находка разбора прав T151); запись пульса монотонна — `UPDATE … WHERE agent_last_seen_at < now` или сравнение, чтобы запоздавший удар `recording:true` не взвёл сторожа (находка разбора схемы T151) | — | 1 |
| T154 | identity | T070,T150 | todo | Приглашение на неподдерживаемую площадку (Контур.Толк, Zoom) отбивается сразу при вставке ссылки: `meeting-invite`/`swarm-api` отвечает понятным отказом EN+RU, веб показывает его под полем. Сейчас веб принимает ссылку, а служба бота потом заводит пустую встречу ради `join_failed` (сдача T070) | — | 1 |
