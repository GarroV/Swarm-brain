...
| **Project Space** (вкладка «Проекты» — сетка проектов + canvas-облако задач с drag-to-connect; заменила вкладку «Граф»/`DependencyGraph`, Task 9) | data-слой `_shared/tasks/projects.ts` (CRUD + `task_count`/`backlog_count`); API `swarm-api/index.ts` (`/projects*`, `/tasks` поля `project_id`/`project_linked`); веб — `miniapp/src/components/tasks/ProjectsGrid.tsx` (сетка проектов), `ProjectSpace.tsx` (канвас одного проекта), `useProjectCanvas.ts` (хук drag-to-connect) | §Таблицы БД (`projects`, `tasks.project_id/project_linked`), §swarm-api |
