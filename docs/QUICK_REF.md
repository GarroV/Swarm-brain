...
- **🔴 Как опознать «CI лежит по биллингу», а не по коду** (случилось 20.08.2026 ~13:00 UTC): job'ы падают **за 3–5 секунд с ПУСТЫМ списком шагов**, `gh run view --log-failed` отдаёт «log not found», а причина видна только в аннотации check-run:
  ...
  ```
  Это аккаунт-уровневая блокировка: раннер не выделяется вообще, перезапуск не помогает (проверено, attempt 3). Пока лежит — проверки гонять локально: **`deno task test`** (из корня: edge-функции + тесты веба; флаг `-A` обязателен — часть тестов установщика пишет в tmp и запускает подпроцесс, под `--allow-read` они падают правами, а не по сути), `deno check`, `tsc --noEmit`, `next build` (pre-commit хук работает независимо). Cloudflare Pages от этого не зависит и деплоит веб как обычно; реально заблокирован только `recorder-release.yml` — новый `.app` в эти дни собирается вручную на Mac.
...
| 🧭 **Пинг задачи (ближайшее напоминание)** — ручная дата напоминания отдельно от срока: «дедлайн 20-го, вспомнить 1-го». Пинг ОДНОРАЗОВЫЙ (отправили → `tasks.reminded_at`, повторов нет), получатель — исполнители, у общей задачи без исполнителя — поставивший (`remind_set_by`), у приватной — владелец (последний рубеж: иначе пинг ушёл бы в никуда и задача висела бы в выборке крона вечно). Перенос пинга ВЗВОДИТ его заново (`reminded_at=null` в PATCH) — иначе передвинутый пинг молча не пришёл бы. Дата — календарный день по Белграду | БД `tasks.remind_date/reminded_at/remind_set_by` (миграция `20260826090000`); крон `swarm-bot/handlers/task-pings{,-send}.ts` (`task_pings_cron`); API `swarm-api/index.ts` (PATCH/POST `/tasks`); веб `miniapp/src/components/{TaskModal,tasks/TaskRow,tasks/TaskQuickActions,roy/NotificationsBell}.tsx` | §Модуль задач, §Крон |
| 🧭 **Регулярные (циклические) задачи** — модель **roll-forward: ОДНА строка катится вперёд** (решение владельца 2026-08-27, канон [decisions/2026-08-27-recurring-tasks.md](decisions/2026-08-27-recurring-tasks.md)); шаблонов и экземпляров на каждое вхождение НЕТ, крона НЕТ. Отметил готовой → срок прыгает на **первое вхождение графика строго позже max(срок, сегодня)** (график не уползает при опоздании), статус снова `open`, `start_date`/`remind_date` сдвигаются на ту же дельту, `reminded_at=null` (пинг взводится заново), в `task_history` — строка «цикл закрыт, следующий срок …» (единственный след: в «Готовых» задача не появится). Частота — `daily`/`weekly`/`monthly`; **день недели и число месяца отдельно НЕ хранятся — это и есть `due_date`**, поэтому цикличность ТРЕБУЕТ срока (строка «Повторять» в карточке задисейблена со значением «нужен срок», API → 400, снять срок у регулярной нельзя). `recur_anchor_dom` — исходное число месяца, чтобы после зажатия коротким месяцем (31 янв → 28 фев) вернуться к 31-му; **пересчитывается только когда человек тронул срок или частоту** (`recurrencePatchFor`) — иначе автосейв `TaskModal` молча увёл бы график с 31-го на 28-е. Перекат живёт в `updateTask` — единственной точке записи статуса, поэтому веб, бот и MCP получают его сразу; бот отвечает «цикл закрыт, следующий срок …», а не «Готово», и показывает частоту в карточке задачи (`🔁 раз в неделю` — намеренно БЕЗ дня недели: таблица русских падежей живёт в вебе, вторая копия разъехалась бы, а точный день виден в строке срока). MCP принимает `recur_freq` в `add_task`/`update_task`. Смарт-список **«Регулярные» скрывается при нулевом счётчике** (кроме случая, когда он активен), из остальных списков регулярные НЕ убираются | БД `tasks.recur_freq/recur_anchor_dom` (миграция `20260827120000`); логика `_shared/tasks/recurrence.ts` (+`.test.ts`, 38 тестов) и `_shared/tasks/db.ts` (`updateTask`); API `swarm-api/index.ts` (POST/PATCH `/tasks`); бот `swarm-bot/tasks/handlers.ts` (`ts_`-callback) + `tasks/db.ts`; веб `miniapp/src/lib/recurrenceLabels.ts` (+`.test.ts`), `lib/smartLists.ts` (`isRecurring`/`HIDE_WHEN_EMPTY`), `components/TaskModal.tsx`, `components/tasks/{TaskRow,SmartListNav,TaskQuickActions}.tsx`, иконка `repeat` в `roy/icons.tsx`; бот `swarm-bot/tasks/formatter.ts` (`recurFreqLabelRu`); MCP `swarm-mcp/tasks/tools.ts` | §Модуль задач, §Таблицы БД, §swarm-api |
| 🧭 **Единый флоу быстрого создания задачи («+»)** — дефолты в ОДНОМ месте, `buildQuickAddInput`: **исполнитель = я всегда** (решение владельца 2026-08-27). До фикса «+» на доске проектов создавал задачу без исполнителя — она по правилу линз попадала в «Команда» и исчезала из «Моих». Общую задачу делают осознанно, сняв исполнителя в полной форме. Контекст добавляет `status`/`project_id`/`sprint_id`, срок «сегодня» (в списке «Сегодня») и `is_private` (под активной меткой; метка перебивает «сегодня») | `miniapp/src/lib/quickAddTask.ts` (+`.test.ts`); зовут `components/tasks/useReminderTasks.ts` (`quickAdd`) и `components/tasks/SprintBoard.tsx` (инлайн-ввод в колонке) | §Модуль задач в [MINIAPP_ARCHITECTURE.md](MINIAPP_ARCHITECTURE.md) |
| 🧭 **Объявление об обновлении + проверка «кто в системе»** — раскатка веба пересобирает Cloudflare Pages, после чего service worker сам перезагружает открытые страницы, поэтому людей предупреждаем плашкой. ⚠️ **Перезагрузка работает только потому, что сборка ШТАМПУЕТ `out/sw.js` отпечатком бандла** (`miniapp/scripts/sw-stamp.mjs`, + `sw-stamp.test.ts`): открытую вкладку будит ТОЛЬКО `controllerchange`, а он срабатывает лишь когда встал новый service worker, то есть когда изменился сам файл `sw.js`. До 28.08.2026 штампа не было — и любая раскатка, не тронувшая `sw.js`, до открытых вкладок не доезжала вовсе (поймано на живом примере: потоковый разбор задач уехал на прод, у владельца открытая с вечера вкладка показывала прежнее поведение). Отпечаток считается от имён и содержимого `out/_next/static`, поэтому пересборка без изменений вкладки не дёргает. Второй половиной фикса вкладка **сама перепроверяет `sw.js`** — при возврате к ней (`visibilitychange`) и раз в 10 минут (`UPDATE_CHECK_MS` в `ServiceWorkerRegister.tsx`): браузер сам перепроверяет только при НАВИГАЦИИ, а в SPA её не бывает, поэтому одного штампа мало, а перед заливкой машинно смотрим, не работает ли кто (решение владельца 2026-08-27, канон [decisions/2026-08-27-deploy-notice.md](decisions/2026-08-27-deploy-notice.md)). Плашка ТОЛЬКО предупреждает (интерфейс не запирается), живёт в вебе — телеграм и рекордер не трогаем. Флаг — строка `app_settings.deploy_notice` `{at, until}`; `until` = срок годности В ДАННЫХ, плашка гаснет сама, если скрипт упал и не снял её. Доставка — прицепом к `GET /notifications` (веб опрашивает раз в 60 с), своего поллинга у плашки НЕТ: узнав `at`, она тикает отсчёт локально. Проверка активности: **запись или обработка встречи → СТОП, `FORCE` не обходит** (оборванный кусок аудио = потерянная встреча); **человек правил что-то за 10 мин → предупреждение**, обходится `FORCE=1`. ⚠️ Признак незавершённой обработки — `meetings.summary_status='processing'`, НЕ `meetings.status` (там `in_base`/`awaiting_review` — состояния вычитки; предикат по `status` матчил 144 строки из 144) | БД `app_settings.deploy_notice`; API `swarm-api/notifications.ts` (`loadDeployNotice`); веб `miniapp/src/lib/deployNotice.ts` (+`.test.ts`), `components/roy/DeployNoticeBar.tsx`, публикует `components/roy/NotificationsBell.tsx`; скрипты `scripts/deploy-{notice,activity}.sh`, цели `make who` / `make notice MIN=15` / `make notice-off`, встроено в `scripts/deploy-window.sh go` | §Деплой, §app_settings, §swarm-api |
...
| Concern | Файлы | Детали |
|---|---|---|
| 🧭 **Охват персонального дайджеста (по чьим странам собирается)** — решает `resolveDigestScope` по НОРМАЛИЗОВАННЫМ кодам (не по сырым `user_profiles.markets`: рынок, который не понял `normalizeCountry`, до фильтра не доедет). Три исхода и НИ ОДНОГО молчаливого: рынки есть → `countries ∩ markets`; админский чекбокс `all_countries` → весь воркспейс; **рынков нет → сервер отвечает `{ needs_markets: true }` и дайджест НЕ строится**. До 2026-08-28 третьего исхода не было — пустой `markets` делал условие фильтра ложным, фильтр молча отключался, и человек получал сводку по всему воркспейсу (в ней доминируют самые многочисленные рынки, поэтому это выглядело как «мне показывают чужой дайджест»). Подтверждено на проде: `markets=[]` у 4 профилей из 12. Текст подсказки живёт в вебе и переводится — сервер отдаёт признак, а не фразу (решение владельца 2026-08-28, issue #154) | сервер `swarm-api/digest-scope.ts` (+`.test.ts`) и блок `POST /digest` в `swarm-api/index.ts`; клиент `miniapp/src/lib/api.ts` (`generateDigest`), `miniapp/src/components/roy/dash/PersonalDigest.tsx`; рынки человек ставит себе в `miniapp/src/components/SettingsScreen.tsx` | §swarm-api |
...
| 🧭 **Классификация стран / теги рынков** (правило + **схлопывание** кросс-маркета: ровно 1 рынок → тег, 0 или **2+** → `General`; порог 3→2 с 2026-08-06, переподтверждён владельцем 2026-08-28). **Единая точка для рынков, пришедших ОТ КЛИЕНТА (чипы, MCP-аргумент, бот-пикер) — `marketTagsFromInput`** (`_shared/meta-extract.ts`): нормализация в ISO + сентинел + порог. Порог применяется и к РУЧНОМУ выбору: чипы предзаполнены подсказкой, поэтому «выбрал человек» на практике часто = «предложила система, человек согласился» (issue #167). Пустой/нераспознанный вход = `["General"]`, а НЕ пустой массив — запись без тега выпадает из дайджеста совсем (issue #166: PATCH встречи прогонял `["General"]` через `normalizeCountries`, который знает только страны, и писал `[]`). До 2026-08-28 порог жил в трёх версиях: swarm-api 2+, MCP 3+ со «списком + General», бот-пикер без порога вообще (issue #169) | `_shared/countries.ts` (`COUNTRY_PROMPT_RULE`, `normalizeCountries` — General ему НЕ словарное значение), `_shared/meta-extract.ts` (`applyGeneralSentinel`, `marketTagsFromInput`); потребители: `swarm-api` (publish + PATCH встречи), `swarm-mcp` (`update_knowledge`), `swarm-bot/handlers/meetings.ts` (`mctry_done_`); дизайн `docs/superpowers/specs/2026-08-06-country-attribution-consolidated.md`, решение [decisions/2026-08-28-market-threshold-stays-2.md](decisions/2026-08-28-market-threshold-stays-2.md) | §Флоу сохранения |

### Встречи — запись → транскрибация → тезисы → ревью
| Concern | Файлы | Детали |
|---|---|---|
| 🧭 **Фильтры «Все встречи» запоминаются** (владелец 2026-08-28: «человек выставил — система запомнила, дальше так и работает») — ключ `localStorage` `roy_meetings_filters_v1`; читается ПОСЛЕ монтирования (статическая страница: инициализатор `useState` дал бы рассинхрон гидрации — тот же порядок, что у периода дайджеста), пишется на каждое изменение, «Сбросить» удаляет ключ. Хранится ПРЕСЕТ периода, а НЕ его границы: их считает `periodBounds()` от сегодня, поэтому «Неделя» месячной давности восстановится свежей неделей (ловушка, уже пойманная на периоде задач); `custom` хранит свои `from`/`to`. Мусор в хранилище не роняет экран — каждое поле валидируется по одному и падает в дефолт | `miniapp/src/lib/meetingsFilters.ts` (+`.test.ts` — форма, дефолты, `parseSavedFilters`/`loadSavedFilters`/`saveFilters`; без импортов экрана, поэтому берётся `deno test src/lib/`), реэкспорт и применение — `components/roy/screens/meetingsFilter.ts`, состояние — `components/roy/screens/MeetAdminScreen.tsx` | §Флоу встреч |
...
| 🧭 **Разбор задач из встречи** — «Сгенерировать» отдаёт предложения (`POST /tasks/extract {save:false}`), и они вычитываются НЕ в узкой панели, а в листе поверх затемнённого экрана (2026-08-27, владелец: «хочется сразу видеть, на что соглашаешься, а не открывать каждую по отдельности»): полный заголовок с инлайн-правкой, контекст, исполнитель/срок/рынок, чекбоксы и ОДНА кнопка «Добавить N задач» (`Promise.allSettled`; неудачные остаются в разборе). Анимация — container transform на `transform`/`opacity` без библиотек: лист «разъезжается» из прямоугольника блока-источника (`--hv-*` из `getBoundingClientRect`). **Строка `"null"` от модели — не значение, а мусор:** гасится и промптом, и `cleanExtractedField` на сервере, и `normalizeProposedTasks` на клиенте (issue #125). **Исполнитель считается ОДНИМ хелпером** `effectiveAssigneeId` (и при публикации, и в строке разбора — иначе человек соглашается не на то, что уедет): имя от GPT резолвится в `telegram_id`, **а не подтянулось по любой причине (не названо / не из команды / неоднозначно) — задача уходит на того, кто её сгенерировал** (решение владельца 2026-08-28, канон [decisions/2026-08-28-assignee-falls-back-to-author.md](decisions/2026-08-28-assignee-falls-back-to-author.md); перекрывает «не нашёлся → без исполнителя» из issue #126). Повод — инцидент #151: две задачи легли ничьими и без срока, то есть мимо «Сегодня», мимо «Мои» и мимо секции автора, при тосте «Добавлено 2 задачи». При подмене строка разбора показывает исходное имя из тезисов рядом с новым исполнителем; ничья задача остаётся только при неизвестной личности разбирающего и считается в хвосте тоста. **Задачи приезжают ПОТОКОМ** (`Accept: text/event-stream` на том же эндпоинте): лист открыт через ~200 мс, первая задача ~через секунду вместо пяти — ожидание это генерация модели, а не инфраструктура (замер: обычный вызов 200–600 мс, извлечение 2.9–5.3 с) | сервер `swarm-api/index.ts` (`extractPrompt`/`callExtractor`/`gptExtractTasks`/`streamExtractTasks`, `cleanExtractedField`), нарезка ответа `_shared/json-object-stream.ts` (+`.test.ts`), протокол SSE `_shared/openai-sse.ts` (+`.test.ts`); клиент `miniapp/src/lib/proposedTasks.ts` (+`.test.ts`), `lib/api.ts` `extractTasksStreamed`, `components/roy/{TasksFromMeeting,TasksHarvestSheet}.tsx`, `.roy-harvest*` в `app/globals.css` | §Флоу встреч, §Модуль задач в [MINIAPP_ARCHITECTURE.md](MINIAPP_ARCHITECTURE.md) |
| **Колонки ответа по `entries` — только `ENTRY_COLUMNS`, `select("*")` ЗАПРЕЩЁН** (`embedding` 18.5 кБ + `fts` 7.8 кБ на строку сервер только пишет и никогда не читает). Списочный `GET /meetings` вдобавок УРЕЗАН (`content`/`summary` до 400 симв., флаг `truncated`); экран, открывающий встречу из объекта списка, обязан до-загрузить её по `id` (issue #102) | канон `swarm-api/entries-guard.ts` `ENTRY_COLUMNS` (+ `.test.ts`); урезание `swarm-api/meetings-payload.ts` (+ `.test.ts`); клиент — `roy/screens/MeetAdminScreen.tsx` `selectItem` | §swarm-api → Форма ответа GET /meetings |
| **Форма ответа `GET /tasks`** — проекция 22 колонки из 35 (1146 → 583 Б/строку) + `X-Total-Count`. `buildPatch()` шлёт ВСЕ поля, поэтому автосейв на урезанной задаче стёр бы `description` (issue #116) → **`TaskModal` догружает полную задачу САМ** и до её приезда держит форму выключенной; отказ показывает полосой с «Повторить». От вызывающих экранов ничего не требуется — прежний 🔴 контракт «догрузи по id» соблюдал 1 экран из 5, остальные молча теряли правки (issue #145) | `swarm-api/task-columns.ts` (+ `.test.ts`); клиент — `isPartial` + догрузка в `TaskModal.tsx`; DEV-моки `lib/api.ts` режут `description` как прод; регресс — `e2e/deep-flows.mjs` | §swarm-api → Форма ответа GET /tasks |
| **Лимиты списочных ответов** — `/tasks` 2000, `/meetings` 500 (потолок 2000), `/entries` и `/agent-meetings` по 50. Лимит режет КОНЕЦ СОРТИРОВКИ: у задач `due_date ASC nulls last` → первыми отваливаются задачи БЕЗ срока (issue #111) | `swarm-api/http.ts` `parseListLimit` (+ `.test.ts`), `TASKS_LIST_LIMIT` в `swarm-api/index.ts` | §swarm-api → Лимиты списочных ответов |
| **Форма ответа `GET /agent-meetings`** — списочный без `draft_notes_md`/`transcript`, вместо текста признак `has_draft_notes` (текст ехал в 10-секундном поллинге: 154 кБ за опрос ≈ 55 МБ/час). Готовность на клиенте — ТОЛЬКО через `hasDraftNotes()`; `summary_status` списком тоже не приходит | `swarm-api/meetings-payload.ts` `toAgentListRow` (+ `.test.ts`), `miniapp/src/lib/agentMeeting.ts` (+ `.test.ts`) | §swarm-api → Форма ответа GET /agent-meetings |
| Дедупликация GET-запросов веба (одна ручка дёргалась 2–8 раз за открытие; TTL 2.5 с < 10 с поллинга; мутация сбрасывает) | `miniapp/src/lib/request-cache.ts` (+ `.test.ts`), подключён в `lib/api.ts` `apiFetch` | spoke [MINIAPP_ARCHITECTURE.md](MINIAPP_ARCHITECTURE.md) |
| Granola импорт / Read.ai / статус-бэкап | `swarm-bot/handlers/granola.ts`, `read-ai-webhook/`, `meeting-status/` | §Флоу встреч |
| 🧭 **Дедуп встреч** (кросс-источниковый: Granola/рекордер/Read.ai). Гейт `identity_key` работает ТОЛЬКО для сравнимых ключей: ключ описывает, как встречу увидел клиент, и живёт в трёх пространствах (`<event>:дата` календаря и `kontur:`/`meet:` комнаты — общие у участников; `granola:<note_id>`, `manual:` — персональные). До 2026-08-28 гейт хватал любую пару непустых ключей и глушил кросс-источниковый дедуп целиком (issue #164 — одна встреча «IT+BD» 26.08 в базе трижды). Эвристика: состав ±5 мин · идентичное название (без окна времени — Granola подключается позже) · участие публикующего в attendees кандидата ±10 мин (для записи из комнаты — у неё нет ни названия, ни участников). Калибровка проверена сухим прогоном по всем 242 записям прода: 13 склеек, 0 ложных. На дубле publish отвечает `duplicate: true`, и веб говорит, что тезисы вычитавшего НЕ сохранены (issue #170) | `_shared/meeting-dedup.ts` (+ `.test.ts`, 26 тестов; `comparableKeys`/`keyScheme`/`normTitle`/`isGenericTitle`); применён в `granola.ts`, `swarm-api` (publish/import), `read-ai-webhook/`; клиентский тост — `miniapp/src/components/roy/screens/MeetAdminScreen.tsx` | §Флоу встреч → Дедуп встреч |
...
| 🧭 **Разрешение на уведомления — читается, а не проглатывается** — уведомления единственный канал, которым рекордер зовёт человека («звонок завершён, сохраняю», «нужен новый токен», «системный звук не пишется»). До 2026-08-28 ответ на `requestAuthorization` отбрасывался (`{ _, _ in }`), `getNotificationSettings` не звался нигде — отказ (или не показанный системой запрос) приложение не замечало НИКОГДА, и все девять `add(UNNotificationRequest)` молча уходили в никуда (жалоба владельца: «у Индиры не прилетают уведомления»). Теперь ответ читается, статус переспрашивается у системы при старте, на пробуждении и по требованию (разрешение снимают в System Settings мимо приложения, помнить первый ответ нельзя). В меню при точно известном отказе — строка «Уведомления выключены — не смогу предупредить»; пункт **«Проверить уведомления»** есть ВСЕГДА: на `.notDetermined` показывает системный запрос (включение в один клик), иначе ведёт в панель настроек с якорем `?id=<bundle id>` — сразу на карточку bumblebee, а не в общий список программ | `AppDelegate.swift` (`setupNotifications`, `refreshNotificationAuthorization`, `checkNotificationsTapped`, `notificationsAllowed`), `Permissions.openNotificationSettings` | повод — [#155](https://github.com/GarroV/Swarm-brain/issues/155) |
| **Обновление рекордера по требованию** — пункт меню «Обновить bumblebee · сборка N»; решение общее с смоуком `--selftest-update` | `Updater.decide` + `AppDelegate.checkUpdateTapped` | spoke [recorder/README.md](../recorder/README.md) → «Обновление по требованию»; повод — [#146](https://github.com/GarroV/Swarm-brain/issues/146) |
| **Токен рекордера: обновление без перевыпуска + перекрытие** — установщик берёт токен из локального конфига; перевыпуск оставляет прежний токен живым до первого входа новым (макс. 24 ч) | `_shared/recorder-token.ts` (`mintRecorderToken`, `revokeRecorderToken`, `buildRecorderUpdateOneLiner`), `_shared/agent-auth.ts` (`classifyToken`) | сквозной прогон `scripts/recorder-token-grace-e2e.ts` (нужна локальная база); повод — [#146](https://github.com/GarroV/Swarm-brain/issues/146) |
...
| `deploy_notice` | `{at, until}` пока объявлено обновление; строки нет = объявления нет |
