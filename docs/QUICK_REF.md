...
| **Форма ответа `GET /meetings`** — какие колонки уезжают в браузер. Списочный ответ УРЕЗАН (`content`/`summary` до 400 симв., флаг `truncated`), `embedding`/`fts` не запрашиваются никогда. Экран, открывающий встречу из объекта списка, обязан до-загрузить её по `id` (issue #102) | `swarm-api/meetings-payload.ts` (+ `.test.ts`); клиент — `roy/screens/MeetAdminScreen.tsx` `selectItem` | §swarm-api → Форма ответа GET /meetings |
