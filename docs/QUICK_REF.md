...
| 🧭 **Строка состояния говорит, ЧТО пишется** (2026-08-28, референс Granola «◎ VC · 49m left»): у значка bumblebee появляется подпись — `Название (≤15 симв.) · 49m left`, после календарного конца встречи `Название · ON AIR`, у звонка без календаря просто `ON AIR`, в покое подписи нет (один глиф). **`0m left` и отрицательный остаток запрещены** — врут; меньше минуты → `1m left`. **15 символов** на название: menu bar не переносит и не сжимает пункты, лишняя ширина выдавливает соседние иконки под чёлку — поэтому режется название, а счётчик остаётся целым. Счётчик обновляет таймер раз в 30 сек и трогает ТОЛЬКО подпись (не `rebuildMenu()`, тот пересобирает всё меню); в покое таймер снимается. Данные не новые: название — `identity?.title`, конец — `scheduledEndAt` (он же автостоп по концу встречи) | `RecorderKit/StatusBarTitle.swift` (+ `Tests/RecorderKitTests`, прогон в CI — XCTest есть только там, где стоит Xcode), `AppDelegate.refreshStatusTitle` | канон [decisions/2026-08-28-status-bar-on-air.md](decisions/2026-08-28-status-bar-on-air.md) |
