...
| 🔔 **Уведомления о комментариях** (колокольчик + пуш в бота). Рассылка — в `POST /tasks/:id/comments`: получатели = исполнители ∪ создатель ∪ владелец задачи, минус автор коммента; **у приватной задачи — только владелец** (иначе исполнитель, который её не откроет, увидел бы заголовок). Круг получателей — чистая функция `commentRecipients` (под тестами), своей копии правила не писать. Пуш best-effort: сбой Telegram не роняет сам комментарий. Клиент **опрашивает** раз в 60 c (realtime недоступен — RLS deny-all, см. §Контроль доступа) | БД `notifications`; API+рассылка `swarm-api/notifications.ts`; правило получателей `_shared/tasks/notify.ts` (+`notify.test.ts`); веб `miniapp/src/components/roy/NotificationsBell.tsx` (смонтирован в 3 точки: полоса «← Главная» в `RoyApp.tsx`, шапка дашборда `RoyDashboard.tsx`, мобильная шапка `screens/SearchScreen.tsx`); deep-link `?task=<id>` — `lib/telegram.ts` (`getDeepLinkTaskId`) → `RoyApp.tsx`; `lib/api.ts` | §Таблицы БД, §swarm-api |
