...
| **Форма ответа `GET /tasks`** — проекция 22 колонки из 35 (1146 → 583 Б/строку) + `X-Total-Count`. 🔴 Экран, открывающий задачу ИЗ СПИСКА, обязан догрузить её по id: `buildPatch()` шлёт ВСЕ поля, поэтому автосейв на урезанной задаче стёр бы `description` (issue #116) | `swarm-api/task-columns.ts` (+ `.test.ts`); клиент — `roy/RoyApp.tsx` `openTask` + `isPartial` в `TaskModal.tsx` | §swarm-api → Форма ответа GET /tasks |
