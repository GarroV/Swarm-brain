...
| **Project Space** (вкладка «Проекты» — сетка проектов → **дерево задач** на react-flow: подзадачи `parent_id` любой глубины, магнит-коннект/drag-to-attach, бэклог-стопка слева; v2 заменила canvas-«облако» v1 и вкладку «Граф») | data-слой `_shared/tasks/projects.ts` (CRUD + `task_count`/`backlog_count`); API `swarm-api/index.ts` (`/projects*`, `/tasks` поля `project_id`/`project_linked`/`parent_id`/`tree_x`/`tree_y` + цикл-защита/каскад); веб (`@xyflow/react` + `d3-hierarchy`) — `ProjectsGrid.tsx` (сетка), `ProjectTree.tsx` (дерево, ленивый `ssr:false`), `treeGeom.ts` (магнит/скорость, юнит-покрыто) | §Таблицы БД (`projects`, `tasks.project_id/project_linked/parent_id/tree_*`), §swarm-api |
