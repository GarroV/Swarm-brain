...
> 🚦 **Раскатка на прод — отдельное действие с отдельным «да» владельца** (решение 24.08.2026, канон — [decisions/2026-08-24-deploy-window.md](decisions/2026-08-24-deploy-window.md)). Окно: обычные правки 09:00–10:00 будни (Белград), миграции и рискованное — ночью. Пуш в `main` считается раскаткой (веб собирается с `main` автоматически). Накопитель: `make deploy-plan` / `make deploy` (`scripts/deploy-window.sh`, метка `prod-deployed`). С 1 сентября, когда вернётся квота Actions, — тот же скрипт в расписание.
