...
| **Веб-интерфейс «Рой»** | Next.js (static export) → Cloudflare Pages, браузер/PWA | Доска задач (Задачи/Таймлайн/**Спринт** — swimlane-секции по проектам + колонка Бэклог), RAG-поиск, вычитка встреч. Вкладка «Проекты» (react-flow дерево) **отключена 2026-08-06** (код паркован) | `swarm-api` |
...
| `GET` | `/projects` | Проекты воркспейса (все участники), с вычисленными `task_count`/`backlog_count` (одним запросом, без N+1) — `_shared/tasks/projects.ts` `listProjects` |
| `POST` | `/projects` | Создать проект `{ name, color?, emoji?, parent_id?, sprint_id? }`; `created_by` = вызывающий. `parent_id` — опционально сделать подпроектом существующего верхнеуровневого проекта; `sprint_id` — вкладка-владелец (валидируется принадлежностью воркспейсу → иначе 400); невалидный `parent_id` (см. `validateParent` в §Таблицы БД → `projects`) → 400 |
| `PATCH` | `/projects/:id` | Обновить `{ name?, color?, emoji?, parent_id?, sprint_id? }` своего воркспейса → 404, если чужой/не найден; `sprint_id` вне воркспейса → 400; невалидный `parent_id` → 400 |
| `DELETE` | `/projects/:id` | Удалить проект своего воркспейса (404, если чужой/не найден). Задачи проекта: `project_id→NULL` (FK `ON DELETE SET NULL`) + явный сброс `project_linked=false` |
| `GET` | `/dependencies` | Bulk: все рёбра зависимостей воркспейса одним запросом (граф, без N+1). Изоляция+приватность: ребро видно только если оба конца видимы вызывающему. ⚠️ Фронтовый потребитель (`DependencyGraph`) удалён (вкладка «Граф» → «Проекты», Task 9) — эндпоинт сейчас без вызывающего в вебе, снос отдельной задачей |
