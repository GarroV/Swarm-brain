...
| `meeting-context` | HTTP POST (desktop-agent) | «Что было в прошлый раз» для панели заметок рекордера ([#226](https://github.com/GarroV/Swarm-brain/issues/226)). Agent-токен (`smcp_`) → страна созвона по явному указанию в названии встречи ЛИБО по однозначному топониму (`_shared/toponyms.ts`, город/регион → страна, issue #229, 23.09.2026; несколько стран в топонимах → отказ) (`_shared/market-suggest.ts`, сигнал `title`; рынки участников из профилей НЕ используются — требование владельца «максимальное совпадение», догадка тянет тезисы чужой страны) → последняя ОПУБЛИКОВАННАЯ встреча этой страны (`entries`, `entry_type='meeting'`, `metadata.confirmed='true'`, `countries @> [страна]`) → превью её тезисов. **Задачи — ВТОРАЯ, независимая выборка: по самой стране** (`tasks.country`), а НЕ по найденной встрече: «задачи и тезисы никак и не должны соприкасаться, это разные вещи» (владелец 03.09.2026). Превью нарезает сервер (`_shared/meeting-context.ts`): заголовки разделов, первые 3 пункта, счётчик, полный текст с потолком 20 000 символов и флагом `truncated` (тезисы на проде в среднем 7 КБ, максимум 24 КБ). Задач по стране на проде: RS 15, SI 13, BG 3 (а вот привязанных к последней встрече обычно нет вовсе — ещё одна причина не связывать секции). Скрыты `done`/`cancelled`/`pending`/`backlog` и неподтверждённые: 32 живых `pending` — это невидимый в вебе импорт из #208. Приватное только владельцу (`is_private=false OR owner_id=<смотрящий>`), воркспейс по `group_id`. LLM НЕТ: тезисы и задачи уже в базе. **Покрытие:** страна распознавалась у 26 из 120 последних встреч (22 %, замер 03.09.2026) — RS 6, BG 5, SI 3, HU 3, MD/HR 2. С 23.09.2026 добавлен словарь топонимов: на той же выборке он даёт ещё 5 встреч (Cluj-Napoca → RO, Catalonia ×2 → ES, Марибор ×2 → SI) без ложных срабатываний. ⚠️ Дальше подход упирается в потолок: **три четверти названий не содержат ни страны, ни города** — «CEE biweekly sync», «Alexandra/Sergey weekly», «IMF BD регулярная встреча каждую неделю». Это и есть регулярные встречи, ради которых блок задуман, поэтому связку «сторона = страна» для них придётся дополнять серией встреч (`identity_key` по uid) или составом участников — разбор в #226, решение не принято. Деплой: `--no-verify-jwt` |
...
| `recorder-diag` | HTTP POST (desktop-agent) | **Журнал рекордера с машин команды** (issue #468, build 35). Рекордер шлёт `kind=log` — строки локального журнала раз в 5 мин, и на старте `kind=session_{abnormal,clean,first_run}` — как закончилась прошлая сессия (маркер `session.json`: `abnormal` = процесс умер, не попрощавшись — падение/SIGKILL/питание), хвост журнала (400 строк) и начало отчёта о падении macOS из `~/Library/Logs/DiagnosticReports`. Пишет в `recorder_diagnostics`, на каждой вставке чистит строки этого пользователя старше 3 дней. Наружу ничего не отдаёт. Зачем: рекордер у коллег «молча пропадал», а heartbeat хранит только последний снимок; просить файлы у людей — не вариант (решение владельца 24.09.2026). Auth — персональный токен |
...
| `project_history` | **Журнал изменений проектов** (issue #426) — по строке на каждое изменившееся поле плюс события жизненного цикла. Заведён 21.09.2026: до него о переездах проектов между пространствами не сохранялось ничего, и раскладку после потери восстанавливали по косвенному следу `created_by` | `id`, `project_id` (FK → `projects`, ON DELETE CASCADE — страховка: проекты теперь архивируются, и журнал переживает архивацию), `group_id`, `field` (колонка проекта — `name`, `space` (это `sprint_id`), `parent`, `owner`, `private`, `start_date`, `end_date`, `color`, `emoji` — либо событие `created`\|`archived`\|`restored`), `old_value`, `new_value`, `changed_by` (**text NOT NULL**, значение всегда подставляет код), `changed_by_telegram_id`, `note`, `created_at`. Индексы: `(project_id, created_at desc)` и `(created_at desc)`. Пишется ОДНОЙ точкой — `_shared/tasks/projects.ts` |
...
| `recorder_diagnostics` | Журнал рекордера с машин команды (issue #468) | `telegram_id`, `build`, `kind` (`log` \| `session_abnormal` \| `session_clean` \| `session_first_run`), `lines` (строки журнала через `\n`, без названий встреч и имён), `crash` (начало отчёта о падении macOS), `created_at`. Индекс `(telegram_id, created_at desc)`. Хранится 3 дня (чистит `recorder-diag`). Миграция `20260924120000`. RLS включён, политик нет. Читать: `select created_at, kind, build, lines from recorder_diagnostics where telegram_id=<id> order by created_at desc` |
| `user_profiles` | Профили пользователей | `telegram_id`, `first_name`, `last_name`, `role` (**CHECK** `role in ('marketing','bd','rnd')`, миграция `20260528120000`), `markets`, `phone`, `email`, `notes`, `name_aliases`, `ui_backdrop` (задник веба, миграция `20260924210000`; NULL = по умолчанию, список проверяет код). ⚠️ **`username` здесь НЕТ** — он в `allowed_users`. Имя = `first_name`+`last_name`, фолбэк на `@username` из `allowed_users` (хелпер `resolveNames` в swarm-api). Не селектить `username` из `user_profiles` — PostgREST упадёт на несуществующей колонке → `data=null` |
| `user_integrations` | API-ключи интеграций | `telegram_id`, `service` (`granola`), `api_key`, `last_polled_at`, `skipped_note_ids` |
| `app_settings` | Глобальные настройки | `key`, `value` — `feedback_channel_id`, `granola_last_polled_at`, `deploy_notice`, `maintenance` (см. §app_settings — ключи) |
...
### Переезд проекта между пространствами и журнал

`PATCH /projects/:id { sprint_id }` переносит проект в другое пространство доски — **вместе с
подпроектами**. Инвариант «подпроект живёт в пространстве родителя» держится везде, где подпроект
создаётся или перетаскивается; переезд самого родителя его ломал, и заметить это глазом нельзя:
доска рисует подпроекты ЧЕРЕЗ родителя, поэтому расхождение всплыло бы много позже.

В интерфейсе — кнопка со стрелкой в заголовке развёрнутого проекта (`MoveProjectMenu.tsx`,
отдельным файлом: `SprintBoard.tsx` и так под 630 строк). Меню показывает остальные пространства
доски и пункт «Без пространства»; выбор сразу двигает проект и его подпроекты в локальном
состоянии, иначе до перезагрузки дети остались бы нарисованными в старом пространстве.

**Журнал** (`project_history`, issue #426) пишется из одной точки — `_shared/tasks/projects.ts`, —
поэтому веб, бот и MCP попадают в него сразу. Что пишется:

- `created` — при создании, в `new_value` имя проекта;
- строка на каждое **реально изменившееся** поле (патч формы несёт весь объект, неизменные поля
  отбрасывает `diffFields`, общий с журналом задач);
- `archived` — при архивации, у родителя в `new_value` число уехавших подпроектов, у детей
  пометка «вслед за родительским проектом»;
- переезд подпроекта вслед за родителем — строкой `space` с той же пометкой.

Ошибка записи журнала **не роняет** саму операцию, но обязательно печатается в лог: пустая история
через месяц неотличима от «никто ничего не двигал» (ровно так уже вышло с задачами, issue #287).
⚠️ Новой таблице нужен явный `grant ... to service_role` — без него вставка возвращает 42501,
операция проходит успешно, а журнал молча пуст; поймано тестом при заведении таблицы.

Читается через `GET /projects/:id/history`. Проверки — `project-history.test.ts` (чистая функция)
и `project-journal.db.test.ts` (настоящая база).
...
один `UPDATE ... SET archived_at = null`.

**Почему.** Удаление в этом продукте уже сработало как тихая потеря данных: снос пространства унёс из
него ВСЕ проекты (`projects.sprint_id` стоит на `ON DELETE SET NULL`), раздел «Проекты» опустел у
всей команды, и восстанавливать раскладку пришлось по косвенному следу — самих строк уже не было.

**Что изменилось в поведении, кроме «строка остаётся»:**

| Операция | Было (DELETE) | Стало (архивация) |
|---|---|---|
| Проект | подпроекты всплывали на верхний уровень (`parent_id` → NULL), задачи отвязывались (`project_id` → NULL) | уходит всё поддерево; `tasks.project_id` сохраняется — вернули проект, вернулся и состав |
...
выборку по этим таблицам — фильтр обязателен, иначе архив полезет в интерфейс.
...
освобождается). Каждый прогнан на намеренно сломанном коде и падает.
...
3. **Браузер — Google Sign-In (ОСНОВНОЙ, 2026-07-29)** — кнопка «Sign in with Google» на `/login`.
   ...
   - **Email-only (без Telegram) РАБОТАЕТ** (issue #21, коммит `ee56ff7`): строка `allowed_users` с email и `telegram_id=NULL` — это ОЖИДАЮЩЕЕ приглашение; при первом Google-входе `auth-resolve` атомарно присваивает ей **синтетическую идентичность `telegram_id = -id`** (реальные Telegram id положительные → коллизий нет) и заводит минимальный профиль (имя из Google, а если его нет — локальная часть email). Такая заглушка при следующем входе с именем от Google заменяется им (`profileNameUpdate`, `_shared/google-profile.ts`); имя, введённое руками, не трогается. Дальше вся система работает с ней как с обычным юзером.
...
| `GET` | `/projects/:id/history` | Журнал проекта (issue #426): до 200 последних строк `project_history`, новые сверху. Доступ — та же проверка, что на правку (`canMutateProject`): чужой закрытый проект → 404, а не пустая лента |
...
| `GET` | `/maintenance` | Идут ли работы: `{maintenance:false}` либо `{maintenance:true, until, message_en, message_ru}`. **Без авторизации, если нет заголовка `Authorization`** — заглушку должен увидеть и тот, у кого протухла сессия, иначе вместо «идут работы» он получит экран входа и решит, что сломался он. Запрос с авторизацией дополнительно несёт `bypass: true` владельцу (он проходит сквозь заморозку). Общий модуль `_shared/maintenance.ts` |
...
| `GET` | `/sprint-cycles` | **Спринты** воркспейса (все участники). `?tab_id=<id>` — спринты пространства, `?tab_id=none` — «Без вкладки», без параметра — все. Модуль `swarm-api/sprint-cycles.ts` |
| `POST` | `/sprint-cycles` | Создать спринт (`name`, `start_date`, `end_date`, `tab_id`, `check_date`) — **любой участник** (решение владельца 18.09.2026). Вкладка чужого воркспейса → 400; второй незакрытый спринт в пространстве → 409 |
| `GET` | `/sprint-cycles/:id` | Спринт + состав (`items`: живой у идущего, из клона у принятого) |
| `PATCH` | `/sprint-cycles/:id` | Правка (`name`, даты, `check_date`, `summary`) — любой участник. **`tab_id`** — перенос спринта в другое пространство (#397): пространство сверяется в ЭТОМ воркспейсе (чужое → 404, иначе спринт пропал бы с экрана команды, оставшись в базе), занятое пространство → **409** (частичный индекс «один незакрытый спринт на пространство»); `null` — «Без пространства» |
| `DELETE` | `/sprint-cycles/:id` | Удаление — **только admin** (единственное действие под админом: необратимо); принятый спринт не удаляется |
| `POST` | `/sprint-cycles/:id/start` | `draft` → `active`: состав фиксируется как ПЛАН (`in_plan=true`) — любой участник |
| `POST` | `/sprint-cycles/:id/accept` | Приёмка **одной транзакцией** через SQL-функцию `accept_sprint_cycle` (`_shared/tasks/sprint-accept.ts`): клон в `frozen_*`, `stats`, следующий черновик в том же пространстве, перенос незакрытых. Любой участник. Не идущий спринт → 409. `next_cycle_id` старого веба игнорируется — следующий спринт создаёт сама приёмка |
| `PATCH` | `/sprint-cycles/:id/tasks/:taskId` | Отметка сверки (`check_status`: `ok`\|`risk`\|`problem`\|null, `check_note`) и пометка «к переносу» (`to_carry`, `carry_reason`). На принятом спринте → 409; снятие пометки чистит причину |
...
| `GET` | `/spaces/:tabId/journal` | **Журнал пространства**: лента событий вкладки (`?days=1\|3\|7\|all`, не больше 800 строк) — правки задач, комментарии, состав спринтов, старт и приёмка. Модуль `swarm-api/space-journal.ts`. ⚠️ Отдаёт **конверт** `{ events: [...] }`, а не голый массив, в отличие от остальных роутов спринтов — клиент разворачивает его в `fetchSpaceJournal` (расхождение роняло экран журнала, починено 19.09.2026). Приватность: сначала список видимых задач, потом события по ним; чужая вкладка → 404. Автор (`actor`): у комментария — `added_by_telegram_id`, запасным `added_by`; числовой автор (старые строки спринтов хранят telegram_id строкой, у Google-входа он отрицательный) при отдаче заменяется именем через `resolveNames` (`swarm-api/journal-actors.ts`), имена и `demo` — как есть |
| `GET` | `/sprints` | ⚠️ Пространства, не спринты. `?kind=board_tab` — пространства доски «Проекты», `?kind=space` — пространства раздела «Спринты»; без параметра отдаёт всё (совместимость с вебом, открытым в момент раскатки). Все участники |
| `POST` | `/sprints` | Создать пространство (`name`, `start_date`, `end_date`, `status`, `kind` — `board_tab` по умолчанию: поверхность называет свою сущность сама) |
...
**⚡ Форма ответа `GET /meetings` — канон `swarm-api/meetings-payload.ts`** (issue #102, 26.08.2026; обрезка переехала в SQL 25.09.2026, issue #490):
...
| **Большой список** (`?confirmed=true` или без параметра) | `ENTRY_LIST_COLUMNS`: `content` и `summary` приходят **из базы уже урезанными** до `LIST_PREVIEW_CHARS` (400 симв.) — generated-колонки `content_preview`/`summary_preview` под прежними именами; у записи стоит **`truncated: true`** (источник — колонка `list_truncated`) |
| **Очередь вычитки** (`?confirmed=false`) | `ENTRY_COLUMNS`: полный текст, без урезания — там единицы строк и текст нужен сразу |

**Где режется — в SQL, а не в JS** (issue #490). Прежняя схема выбирала полный транскрипт и обрезала его в Deno: база отдавала 3.6 МБ ради ответа в 440 кБ. Замер на проде 25.09.2026 (283 видимых встречи, `explain analyze`): полные колонки — **654 мс**, `left(content, 400)` — **15 мс**. PostgREST не умеет SQL-функции в `select`, поэтому превью живут как generated-колонки; ответ при этом не изменился ни на байт. Обрезка в `toListRow` осталась как последний рубеж на случай запроса с полными колонками. Число 400 продублировано в миграции `20260925090000` и в `LIST_PREVIEW_CHARS` — дрейф ловит `list-preview.db.test.ts` на живой базе.
...
| Метод | Путь | Что делает |
|-------|------|-----------|
| `GET` | `/agent-meetings` | Очередь вычитки / опубликованные (`?status=awaiting_review\|in_base`). Видны **только владельцам черновика**: записавшим (`recorders`) и совладельцам (`co_owners` — участники встречи с аккаунтом SWARM, найденные по e-mail приглашения в том же воркспейсе; считаются в `meeting-claim`, `_shared/meeting-owners.ts`). Админ НЕ исключение (решения владельца 2026-08-20 и 2026-09-25). Правило — `_shared/meeting-access.ts`. `DELETE` черновика — только записавший (403 совладельцу); `publish` с `base=personal` у черновика с несколькими владельцами — 409, только в общую базу. Прежний `?all=true` убран; пригляд — агрегат `GET /admin/review-counts`. **Ответ СПИСОЧНЫЙ:** без `draft_notes_md`/`transcript`, вместо текста тезисов — признак `has_draft_notes` (issue #108) |
...
├── identity.ts         # withTokenIdentity: схемы не просят личность — она из токена (#502)
├── meetings.ts         # вычитка черновиков и извлечение задач (#513, #514)
├── meetings-format.ts  # форматирование ответов вычитки (+.test.ts)
└── tasks/
    ├── tools.ts        # Прослойка: резолв user/assignee → _shared/tasks/db.ts → форматирование строк
    ├── analytics.ts    # статистика и журнал задач
    └── format.ts, analytics-format.ts  # форматирование (+.test.ts)
```
...
| `whoami` | Кто вызывающий по токену: имя, внутренний ID (`telegram_id`, у веб-пользователей без Telegram — синтетический отрицательный), воркспейс, роль владельца (issue #502) |
| `get_users` | Список пользователей воркспейса, **включая владельца** (до 25.09.2026 он был исключён без причины — агент не находил его в команде, issue #502). Приглашённые без `telegram_id` не показываются; фильтр `market` — по массиву `markets` в коде, без учёта регистра (`.ilike` по `text[]` ронял запрос) |
...
| `get_task_comments` | Комментарии-апдейты к задаче по её ID (если задача доступна вызывающему). ID — полный uuid из строки `get_tasks`. У каждого комментария печатается его `id` (для `delete_task_comment`) |
...
| `add_task_comment` | Добавить комментарий-апдейт к задаче по её ID от лица вызывающего. ⚠️ Уведомления и подписку, в отличие от веба, не шлёт (issue #521) |
| `delete_task_comment` | Удалить СВОЙ комментарий (issue #515). Правило — `commentDeleteDenial` в `_shared/tasks/comments.ts`, общее с вебом; в MCP админского обхода нет. Правки текста нет сознательно: у комментария нет отметки «изменено», а уведомление со старым текстом уже ушло |
| `get_review_queue` | Очередь вычитки: свои черновики рекордера (`meetings.status=awaiting_review`, фильтр «свои» = записывал ИЛИ совладелец, `draftMeetingsOwnScopedFilter` — как у `GET /agent-meetings`), с `id`; обрезанная выдача говорит об этом (issue #513) |
| `get_draft_meeting` | Черновик: название, время, участники, тезисы. Доступ — `canAccessDraftMeeting` (только записавший, админ не исключение) |
| `update_draft_meeting` | Правка тезисов (`notes`, ставит `notes_edited_at`) и/или названия до публикации — как `PATCH /agent-meetings/:id` |
| `publish_draft_meeting` | Публикация в базу (`base`: team/personal, `countries`) — тем же `publishDraftMeeting`, что веб: дедуп, арбитраж полноты, рынки, эмбеддинг. Ответ говорит, склеилась ли встреча с уже лежащей в базе и чья версия осталась. Встречу нескольких владельцев в личную базу не пускает (409) — проверка внутри `publishDraftMeeting`, общая с вебом |
| `extract_tasks_from_meeting` | Предложения задач по тезисам черновика (`meeting_id`) или записи (`entry_id`) — экстрактор `_shared/task-extract.ts`, общий с `POST /tasks/extract`. **Ничего не создаёт.** Исполнитель сверяется с командой (`matchAssignee`); не нашёлся или не назван → по правилу проекта задача на разбирающего ([решение](decisions/2026-08-28-assignee-falls-back-to-author.md)), выдача так и пишет (issue #514) |
...
**Личность вызывающего — из токена, агент её не передаёт** (issue #502). `requesting_user_id`/`owner_telegram_id` остались в схемах только для soft-режима, но в `tools/list` они проходят через `withTokenIdentity` (`swarm-mcp/identity.ts`): снимаются из `required`, а описание говорит «заполняется из токена, не передавай». Раньше схемы требовали «твой Telegram ID», и агент без Telegram-номера в контексте не мог вызвать, например, `add_task_comment`. Новый инструмент с этими полями получает верное описание автоматически.
...
| `maintenance` | `{until, started_at, message_en, message_ru}` (ISO + тексты) | **Заморозка системы на время работ** — заглушка «идут работы» в вебе и отказ на любое ИЗМЕНЕНИЕ (503 + `Retry-After`); чтение работает, владелец проходит. Отличие от `deploy_notice`: та предупреждает, эта запрещает. `until` — срок годности В ДАННЫХ (потолок 180 мин): режим гаснет сам, даже если скрипт упал. Ставит/снимает `scripts/maintenance.sh` (`make freeze` / `make unfreeze`). Отсутствие строки = заморозки нет. Канон — [decisions/2026-09-25-maintenance-freeze.md](decisions/2026-09-25-maintenance-freeze.md) |
