...
| `get_review_queue` | Очередь вычитки: свои черновики рекордера (`meetings.status=awaiting_review`, фильтр «только свои» — как у `GET /agent-meetings`), с `id`; обрезанная выдача говорит об этом (issue #513) |
...
| `publish_draft_meeting` | Публикация в базу (`base`: team/personal, `countries`) — тем же `publishDraftMeeting`, что веб: дедуп, арбитраж полноты, рынки, эмбеддинг. Ответ говорит, склеилась ли встреча с уже лежащей в базе и чья версия осталась |
