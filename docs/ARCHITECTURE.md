...
| `projects` | Проекты (Рой) — **теперь = пользовательские секции доски «Спринт»** (`SprintBoard.tsx`, владелец создаёт «+ Секция» → `createProject`). Задача в секции через `tasks.project_id`, колонка через `tasks.status` (`backlog`/`open`/`in_progress`/`done`) | `id` (uuid PK), `group_id` (FK → `workspaces.id`), `name`, `color`, `emoji`, `created_by` (bigint, telegram_id, nullable), `created_at`. Удаление — `deleteProject()`: FK `ON DELETE SET NULL` обнуляет `project_id` у задач + сброс `project_linked=false`. Миграция `20260801120000`. ⚠️ Поля `tasks.parent_id`/`tree_x`/`tree_y` (миграция `20260806120000`) относились к отключённой вкладке-дереву «Проекты» — **паркованы**, в текущем UI (секции) не используются |
