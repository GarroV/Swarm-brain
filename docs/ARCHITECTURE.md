...
| `POST` | `/projects` | Создать проект `{ name, color?, emoji?, parent_id? }`; `created_by` = вызывающий. `parent_id` — опционально сделать подпроектом существующего верхнеуровневого проекта; невалидный `parent_id` (см. `validateParent` в §Таблицы БД → `projects`) → 400 |
| `PATCH` | `/projects/:id` | Обновить `{ name?, color?, emoji?, parent_id? }` своего воркспейса → 404, если чужой/не найден; невалидный `parent_id` → 400 |
