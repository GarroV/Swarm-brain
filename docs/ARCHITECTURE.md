...
`granola_poll`) читает и `allowed_users.recorder_last_*`, и `service_agents.last_*`: решение и адресат —
`swarm-bot/lib/recording-watchdog.ts`, запросы — `recording-watchdog-store.ts`. Бот, замолчавший
дольше 10 мин (`AGENT_STALE_MIN`, удар раз в 2 мин) на `last_recording=true`, даёт алерт EN+RU
человеку, за которого писал: `claim_owner` самой свежей встречи с `identity_key = last_meeting_key`.
...
(`last_seen_at` равен прочитанному), свежий удар между чтением и сбросом алерт отменяет. Смоук —
`scripts/scriba-watchdog-smoke.ts`.

**Известный разрыв:** строка `service_agents` одна на агента. Два контейнера на двух встречах пишут
в неё по очереди, и живой своими ударами прячет замолчавший. Различать удары по встрече нельзя без
новой колонки или таблицы — ждёт решения по схеме (см. `docs/furca/blocks/orchestrator.md`).
