...
| Рекордер публикация | `swarm-api` `POST /agent-meetings/:id/publish` | привязать `meeting` к существующей записи + вернуть её с флагом **`duplicate: true`** (только если она видима публикующему — публичная/его личная; чужие приватные игнорируем). Флаг обязателен: в базе остаётся ЧУЖАЯ версия тезисов, а правки вычитавшего никуда не уехали — веб говорит об этом прямо, вместо прежнего «Черновик опубликован» (issue #170) |
