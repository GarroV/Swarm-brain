...
> `/help` — единый текст со ссылками (без inline-кнопок): веб **Swarm** — HTML-ссылка, команды Telegram делает тапабельными сам. Раздел рекордера включает шаг привязки Google-календаря (Swarm → Настройки). Текст — `handlers/help.ts` `getHelpText()`.
