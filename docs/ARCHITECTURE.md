...
| `GET` | `/spaces/:tabId/journal` | **Журнал пространства**: лента событий вкладки (`?days=1\|3\|7\|all`, не больше 800 строк) — правки задач, комментарии, состав спринтов, старт и приёмка. Модуль `swarm-api/space-journal.ts`. Приватность: сначала список видимых задач, потом события по ним; чужая вкладка → 404 |
