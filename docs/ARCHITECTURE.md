...
| `meeting-heartbeat` | HTTP POST (desktop-agent) | Heartbeat рекордера: раз в ~15 мин + при старте/смене статуса записи (`recording:true/false`) пишет `allowed_users.recorder_last_{seen,recording,version}`. С 04.09.2026 несёт ещё **присутствие** для `ON AIR` в панели «Встречи сегодня» — `on_call` (идёт реальный созвон: вход микрофона держит другое приложение) и `meeting_key` (`<uid>:<дата>` встречи) → `recorder_last_{on_call,meeting_key}`; ключ хранится только пока звонок или запись идут, иначе `NULL` — иначе он завис бы после созвона. Пока звонок идёт, рекордер шлёт keep-alive раз в 2 мин (`RecorderKit.PresenceBeacon`), панель считает присутствие живым 5 мин. Читается watchdog'ом `checkRecorderHealth` (swarm-bot, из `sweepStuckMeetings`-cron) для 2 сигналов: **оборванная запись** (`recording=true` + молчит >20 мин = краш во время записи → алерт записавшему; у бота scriba — `meetings.agent_last_*` каждой встречи (D018), порог 10 мин, алерт `claim_owner` встречи, см. [§Бот scriba](#бот-scriba-оркестратор-встреч-botsrcorchestrator)) и **истечение токена** (<7 дней → алерт `/recordertoken`). Заменил ложный Read.ai-watchdog «встречи не поступают». Auth — личный токен человека ИЛИ токен служебного агента с заголовком `X-On-Behalf-Of` (см. [Личность служебного агента](#личность-служебного-агента)); **heartbeat агента не ложится в `allowed_users`** — иначе watchdog принял бы бота за рекордер человека и погасил настоящий сигнал «запись оборвалась» (решение D007). С D018 удар агента несёт `meeting_id` и пишется в строку встречи (`meetings.agent_last_{seen_at,recording}`) — условиями той же UPDATE сверяется, что встреча в воркспейсе агента и её `claim_owner` — человек из `X-On-Behalf-Of`; ни одной строки → **403** (чужая и несуществующая не различаются); `recording:true` без `meeting_id` → **400**, не-uuid → **400**. Плюс строка агента `service_agents.{last_seen_at,last_version}`. Какие поля, куда и с какими условиями — чистая функция `meeting-heartbeat/write.ts` (`buildHeartbeatWrites`) |
| `meeting-notice` | HTTP POST (бот-участник `scriba`) | **Громкий отказ доходит до человека** (блок `notices`): бот в контейнере токена Telegram не имеет, поэтому приносит сюда причину, по которой запись НЕ идёт, а сервер шлёт её владельцу встречи тем же Telegram, что и `meeting-processor`. Тело: `{kind, meeting_id | meeting_key, title?, lang?, detail?}`, девять видов отказа — `door_waiting`, `door_denied`, `captcha`, `no_conference_link`, `no_owner`, `no_audio`, `recording_lost`, `container_died`, `join_failed` (`_shared/notices.ts`; тексты en/ru — `_shared/notice-texts.ts`, по умолчанию en). **Привязка к встрече:** встречные виды (всё, кроме `no_conference_link`/`no_owner`) несут `meeting_id` из `meeting-claim`; сервер проверяет, что встреча есть (иначе 404), из воркспейса агента и что уведомляемый — её владелец `claim_owner` (иначе 403); название берётся из встречи, присланное `title` → 400. До-встречные виды случаются до claim и несут ключ календаря `meeting_key` (+ `title`). **Получатель — всегда человек из `resolveActingIdentity` и никогда из тела запроса**, иначе токен бота стал бы рассылкой. **Потолок решает база по журналу `meeting_notices`**, присланный `attempt` → 400: функция `meeting_notice_reserve` под блокировкой получателя (`pg_advisory_xact_lock`) считает журнал, решает и заводит строку `sending` одной транзакцией — параллельные вызовы по разным встречам и видам потолок не обходят; отказ → 409. Строка `sending` старше 5 мин считается недоставленной. Дверь — первое уведомление (`next_reminder_in_s: 180`) и единственный повтор (`should_leave: true`), дальше 409; прочие виды — по одному на встречу; сверху 6 на встречу, 30 на человека за сутки, из них до-встречных 6. Недоставленное сообщение — **502 с причиной**, строка журнала → `failed` и номер свободен (успех Telegram — только `ok: true` в теле). Прогон всех сценариев отказа — `scripts/scriba-notices-smoke.ts` (против локального контура). Auth — личный токен человека ИЛИ токен служебного агента с `X-On-Behalf-Of`. Деплой: `--no-verify-jwt` |
| `meeting-invite` | HTTP POST (оркестратор бота `scriba`) | **Оркестратор забирает приглашения бота своего воркспейса** (D017): ожидающие (не забраны, не погашены, не истекли) строки `meeting_invites`, старые первыми, каждая отдаётся ровно раз — забор одним условным UPDATE (`taken_at is null`), два одновременных опроса строку не делят. Тело необязательно: `{limit?: 1..20}` (по умолчанию 10). Ответ `{ok, invites: [{id, invited_by, join_url, platform, created_at, expires_at}]}`. Auth — **только токен служебного агента и без `X-On-Behalf-Of`** (дверь `resolveServiceAgent`; личный токен → 401, заголовок подмены → 403, агент без воркспейса → 403). Деплой: `--no-verify-jwt`. См. [Приглашение бота](#приглашение-бота-d017) |
...
| `service_agents` | **Служебные агенты (боты)**, ходящие на сервер своим токеном и действующие от имени человека. Отдельно от `allowed_users` намеренно: та — таблица ЛЮДЕЙ, и служебная строка в ней молча сделала бы бота сотрудником в рассылках, дайджесте и выборках команды (решение D007). Первый жилец — `scriba` | `id` (TEXT PK — слаг агента), `name`, `group_id` (FK → `workspaces.id`; агент действует ТОЛЬКО за человека из своего воркспейса), `token_hash` (UNIQUE, sha256-hex; сам токен живёт в секретах контейнера), `token_expires_at` (NULL = бессрочный), `is_active` (выключатель без удаления строки), `created_at`; heartbeat агента «жив, такая-то сборка»: `last_seen_at`, `last_version` (миграция `20260917120000`); `last_recording`, `last_meeting_key` с D018 не пишутся и не читаются — удар по встрече живёт в `meetings.agent_last_*`, колонки снимаются вторым шагом. RLS включён, политик нет (внешний замок) |
| `meeting_notices` | **Журнал уведомлений бота `scriba` об отказе записи** (блок `notices`) — источник правды для потолка повторов: счёт ведёт сервер, а не бот | `id` (bigserial PK), `meeting_id` (FK → `meetings.id`, встречные виды) **или** `meeting_key` (ключ календаря до-встречных видов, без FK — строки встречи ещё нет; ровно одно из двух — check), `recipient_id` (FK → `allowed_users.telegram_id`, всегда человек), `kind`, `attempt` (1–2), `status` (`sending` → `sent` / `failed`), `sent_at`. Пишет и решает функция `meeting_notice_reserve(p_recipient, p_meeting_id, p_meeting_key, p_kind, p_limits)` — счёт, потолки (величины из `_shared/notices.ts` `NOTICE_LIMITS`) и вставка под advisory-блокировкой получателя; EXECUTE только у `service_role` (с PUBLIC снят). Частичные уникальные индексы на (встреча|ключ, получатель, вид, попытка) без `failed` — страховка; индекс `(recipient_id, sent_at)` для суточного счёта. У `service_role` нет `delete` — историю не переписывают (миграция `20260926080446`). RLS включён, политик нет |
| `meeting_invites` | **Приглашения бота на созвон** (D017): человек вставил в вебе ссылку — это одноразовое короткоживущее приглашение, единственное основание, по которому служебный агент заводит ручную встречу | `id` (uuid PK), `group_id` (FK → `workspaces.id`), `invited_by` (FK → `allowed_users.telegram_id` — кто позвал; бот обязан действовать именно за него), `join_url` (ссылка, проверенная сервером: https, Meet / Контур.Толк / Zoom, без учётных данных и фрагмента), `platform` (`meet`\|`kontur`\|`zoom`), `created_at`, `expires_at` (+15 мин, `INVITE_TTL_MS`), `taken_at`/`taken_by` (FK → `service_agents.id`; оркестратор забрал в работу), `used_at` (погашено заявкой бота; условный UPDATE — из параллельных заявок проходит одна), `meeting_id` (FK → `meetings.id`, `on delete set null`; встреча по приглашению). Частичный индекс `(group_id, created_at) where taken_at is null and used_at is null`. Гранты только `service_role`, с `anon`/`authenticated` сняты (миграция `20260926124606`). RLS включён, политик нет |
...
## Личность служебного агента

Служебный агент (`scriba` — бот-участник видеовстреч) приходит со **своим** токеном, но действует **от
имени человека**. Вся эта власть собрана в одной функции — `_shared/agent-auth.ts` `resolveActingIdentity`.

| Что | Как |
|---|---|
| Где токен | `service_agents.token_hash` (sha256-hex). В `allowed_users` служебных строк нет |
| Как указывается человек | HTTP-заголовок `X-On-Behalf-Of: <telegram_id>` (или аргумент `onBehalfOf` — для вызова, где значение пришло телом). Названы оба и расходятся → 403 |
| Что на выходе | `AgentIdentity { telegramId, groupId, kind, agentId? }`, где `telegramId` — **всегда человек**. Поэтому `claim_owner`, `owner_id` и всё остальное остаются человеческими, а вызывающий код не знает, что пришёл бот |
| `kind` | `recorder` \| `recorder_prev` \| `mcp` \| `bot` |

Правила, закрытые блокирующими тестами (`_shared/agent-auth-resolve.test.ts`, `meeting-claim/ownership.test.ts`):

- токен агента **сам по себе не даёт прав ни на что** — без `X-On-Behalf-Of` это 403;
- личный токен человека (`recorder`/`recorder_prev`/`mcp`) действовать за другого **не может** — 403, включая «за самого себя»;
- указанный человек обязан существовать и быть **в воркспейсе агента**; человек без воркспейса и агент без воркспейса — отказ;
- мусор в заголовке — внятный 403, а не молчаливое игнорирование.

**Граница — состав встречи, а не весь воркспейс** (решение D016, issue #371). Воркспейса мало:
скомпрометированный или ошибившийся токен иначе заводил бы приватную запись человеку, которого на
звонке не было. Поэтому `meeting-claim` сверяет агента ещё и с составом встречи — там, где состав
известен (`meeting-claim/agent-scope.ts`, блокирующие тесты `meeting-claim/agent-scope.test.ts`):

Всё, что агент прислал о встрече, — его собственное утверждение, поэтому **тип встречи сервер определяет
сам, по форме ключа**, а состав берёт из того, что знает сам:

| Форма `identity_key` | Допустимый `identity_kind` агента | Что проверяет сервер | Состав встречи |
|---|---|---|---|
| `<iCalUID\|id>:<YYYY-MM-DD>` (собирает `calendarKeyOf` из `_shared/calendar-key.ts` — одна функция для `meeting-current` и сверки в `meeting-claim`, рядом её регулярка `CALENDAR_KEY`) | только `calendar` | событие есть **в Google-календаре названного человека**; нет → 403, календарь не подключён → 403 с подсказкой запустить бота руками, Google не ответил → 503 | из события Google, тело запроса не в счёт |
| `meet:<код>` / `kontur:<комната>` (формы рекордера, возможно с суффиксом `:<дата>`) | только `room` | ничего сверх воркспейса | пустой — из тела не берётся |
| любая другая | только `manual` | **действующее приглашение человека** (D017): `invite_id` есть в `meeting_invites`, позвал именно названный человек из воркспейса агента, не истекло, не погашено, `join_url` — та же комната (хост+путь, `?hl=en` не мешает); иначе 403 одним текстом на все причины. Приглашение гасится одним условным UPDATE до создания встречи | пустой — из тела не берётся |
| комната недопустимой формы, несовпадение формы и типа | — | 400 | — |

Встать в уже открытую встречу (тот же ключ или склейка по составу) агент может, только если она его
человека: **календарная — есть в календаре человека** (состав строки не в счёт), комнатная — человек в её
составе (`attendees[].email`, регистр не важен), либо человек уже держит её (`claim_owner`). Иначе
склейка не происходит (заводится своя встреча), а конфликт точного ключа — 403. `zoom:<id>` станет
комнатой агента вместе с T121.

Людей (рекордер, MCP) сверка не касается: их токен и есть их личность. Остаточный риск, принятый D016:
комнатная встреча без состава открыта агенту любого человека своего воркспейса. Ручная — только по
приглашению (D017, ниже).

**Подмена делается ОДИН раз, на входе**, а не протаскивается параметром через места вызова: среди них
есть запись `owner_id`, и протаскивание означало бы, что приватность встречи зависит от того, не забыл
ли кто-то параметр в десятом месте.

⚠️ **`verifyAgentToken` — дверь только для людей и должна ею остаться.** По ней ходит `meeting-webtoken`,
печатающий JWT браузерной сессии на семь дней: агент, прошедший там «от имени человека», перестал бы быть
записывающим и стал бы этим человеком в вебе — на неделю, а не на одну операцию.

Через `resolveActingIdentity` ходят **все пять эндпоинтов, нужных боту**: `meeting-current`,
`meeting-claim`, `meeting-ingest`, `meeting-heartbeat`, `meeting-status`. Какой эндпоинт какой
дверью проверяет вход, закреплено структурным тестом `_shared/auth-doors.test.ts` — правку
«давай везде одинаково» не поймал бы ни один поведенческий тест, потому что для человека
поведение не меняется.

Токен агента заводится вручную: строка в `service_agents` с `token_hash = sha256(токен)`; сам токен — в
секреты контейнера агента, в базу и в репозиторий не попадает.

### Приглашение бота (D017)

Ручной запуск — как кнопка Read.ai: человек вставляет в вебе ссылку на созвон, и бот стучится. **Вставка
залогиненным человеком И ЕСТЬ приглашение**: без него служебный агент ручную встречу не заводит, иначе
украденный токен бота заводил бы человеку приватную запись встречи, куда его никто не звал, в обход
календарной сверки D016. Основной путь остаётся календарным (T100); этот — постоянный запасной (D015).
...
**Веб (поверхность человека).** Карточка «Позвать бота на созвон» / «Invite the bot to a call» — на экране
«Встречи» (таб `cal`, `RoyMeetingsScreen`): на десктопе над лентой, на мобайле под очередью вычитки. Поле
ссылки → `POST /meeting-invites`; ниже — свои приглашения со статусом: «Ждёт бота» (`pending`) · «Бот стучится
в звонок» (`taken`) · «Бот записывает» (`used`, кнопка «Открыть встречу» → вычитка `?meeting=<meeting_id>`) ·
«Истекло» (`expired`). Пока статус не окончательный (`used`/`expired`), веб переспрашивает
`GET /meeting-invites/:id` раз в 5 с. Списка своих приглашений сервер не отдаёт, поэтому id (до трёх) веб
помнит в `localStorage` (`swarm.meetingInvites`) — это удобство вкладки, а не источник истины: 404 (в том
числе приглашения другого воркспейса после входа в демо) убирает строку. Отказы показываются текстом по
`code` на языке интерфейса (`useDt`), браузерная проверка URL выключена, чтобы мусор получил тот же текст.
Код: разбор ответа, статусы, тексты — `miniapp/src/lib/meetingInvite.ts` (+ `.test.ts`), клиент —
`createMeetingInvite`/`fetchMeetingInvite` в `lib/api.ts` (`ApiError.body` несёт тело отказа), экран —
`components/roy/InviteBotCard.tsx`.

## Говорящие в стенограмме (`_shared/speakers.ts`)

Рекордер `bumblebee` пишет две дорожки и знает про говорящих ровно две вещи: «я» (микрофон владельца)
и «собеседник» (всё, что играло через систему). Бот `scriba` сидит на встрече как участник: дорожка у
него одна, зато площадка сама показывает, **кто** говорит сейчас. Поэтому у формы `meeting-ingest`
появилось **необязательное** поле `speakers` — таймлайн говорящих.

```jsonc
// поле speakers формы meeting-ingest, JSON-строка; времена — секунды от начала записи
[{ "start": 0.0, "end": 12.4, "name": "Василий Гарро" }]
```

| Что | Как |
|---|---|
| Где живёт | `meetings.process_state.speakers` — **та же jsonb-колонка**, что и манифест частей. Миграции нет: поле необязательное, старые строки читаются как «таймлайна нет» |
| Валидация | `parseSpeakerTimeline` — **после проверки владения встречей** (иначе держатель токена агента платил бы разбором за чужой и даже несуществующий `meeting_id`) и всё ещё **до любых записей**. Отвергает 400-й: строка длиннее `MAX_SPEAKER_TIMELINE_CHARS` (проверяется **до** `JSON.parse`, как `OPENAI_AUDIO_MAX_BYTES` отбивает часть аудио до чтения), не-JSON, не массив, не объект в элементе, `name` не строка / пустое / длиннее 120 символов, `start`/`end` не конечные числа, `start < 0`, `end <= start`, больше 5000 интервалов. Текст ошибки несёт индекс: `speakers[3]: end must be greater than start` |
| Чистка имени | Пробельное (в т.ч. перевод строки) → один пробел; **невидимое удаляется**, а не заменяется пробелом, иначе имя разорвётся («Ан на» вместо «Анна»). Снимаются C0/C1, DEL, мягкий перенос, zero-width, метки направления и bidi-оверрайды, изоляты, блок Unicode Tag. Класс собран из списка `INVISIBLE_RANGES`, а не написан литералом: литерал этих символов в исходнике сам невидим — его не найти grep-ом и легко затереть правкой |
| Коллизия со служебной меткой | Имя, совпавшее с `«я»` или `«собеседник»` (без учёта регистра, после чистки — так что `«Я»`, `« я »` и `«я»` с zero-width внутри ловятся тоже), получает суффикс `RESERVED_LABEL_SUFFIX`. **Не 400:** имя задаёт посторонний участник, и отказ означал бы, что любой участник одним ником срывает загрузку уже записанной встречи. Без этого участник, назвавшийся `«я»`, получал бы на дорожке `sys` метку владельца записи — подделка авторства в тексте, который человек и модель читают как факт |
| Сведение | `nameAt(timeline, start, end)` — наибольшее **строго положительное** перекрытие; ничья → более ранний интервал; касание границ перекрытием не считается; сегмент нулевой длины → интервал, содержащий точку |
| Куда подставляется | `Segment.speaker` дорожки **`sys`**. Дорожка `mic` всегда остаётся «я»: это микрофон `claim_owner` по построению (чужой аплоад отбивается 403), и он авторитетнее таймлайна; у бота дорожки `mic` не бывает вовсе |
| Мягкая деградация | Нет поля / пустой таймлайн / на сегмент не нашлось имени → **прежняя метка** «собеседник». Не ошибка. `bumblebee` о поле не знает и знать не должен |

**Легенда говорящих в промпте тезисов** (`speakerLegend`) строится по меткам, которые в стенограмме
**реально есть**. Если именованных меток нет — возвращается прежняя строка дословно, чтобы записи
`bumblebee` сводились ровно как раньше. Если имена есть — легенда перечисляет только присутствующие
метки: обещание «я — владелец записи» там, где ни одной реплики «я» нет (запись бота — только имена),
сбивает модели атрибуцию.

Модуль чистый — ни сети, ни `Deno.env`, ни базы; покрыт `_shared/speakers.test.ts`. Ошибка тут
...
onBehalfOf, invite)`; веб-поле для вставки ссылки — `miniapp/src/components/roy/InviteBotCard.tsx` (T150, подпункт «Веб» ниже). Контракт и состояние — [блок
...
`swarm-bot/lib/recording-watchdog.ts`, запросы — `recording-watchdog-store.ts`. Встреча, по которой бот
замолчал дольше 10 мин (`AGENT_STALE_MIN`, удар раз в 2 мин) на `agent_last_recording=true`, даёт алерт
EN+RU `claim_owner` этой же встречи. Тишина считается **по каждой встрече** (D018): при двух встречах
бота сразу живой контейнер не прячет замолчавший.
Если по этой встрече этому человеку уже ушла нотиса `container_died` (`meeting_notices`, `status=sent`)
— второй раз не пишет: сторож — страховка на случай, когда умер и оркестратор. Сброс флага условный
(`agent_last_seen_at` равен прочитанному), свежий удар между чтением и сбросом алерт отменяет. Смоук —
`scripts/scriba-watchdog-smoke.ts`.

**Долгая встреча бота не становится «призраком».** Тот же cron в `sweepStuckMeetings` метит
`summary_status='failed'` пустые встречи старше 15 минут (claim был, ingest нет) — это рассчитано на
рекордер человека, у которого claim и ingest идут рядом. Бот заявляет встречу до захода и отдаёт запись
только в конце, поэтому встреча со свежим (≤ `AGENT_STALE_MIN`) heartbeat агента по ней
(`meetings.agent_last_seen_at`) призраком не считается — и вторая одновременная встреча бота тоже. Критерий — одна функция
`botKeepsMeetingAlive` в `swarm-bot/lib/ghost-sweep.ts` (запросы — `ghost-sweep-store.ts`); пометка
условная — встреча всё ещё пустая, иначе пришедший ingest не затирается. Замолчавший бот и призраки
рекордера метятся, как раньше; `failed` не удаляет строку, и запись, дошедшая позже, обработается.

**Известный предел:** если claim встречи посреди записи перехватит рекордер человека (арбитраж по
`recorded_seconds`), удары бота начнут получать 403, а `agent_last_recording=true` останется — сторож
пришлёт новому `claim_owner` алерт «scriba перестал отвечать». Кто должен гасить флаг при перехвате —
не решено.
...
_Приглашение бота на созвон — `/meeting-invites` (D017, [Приглашение бота](#приглашение-бота-d017)); код `swarm-api/meeting-invites.ts`:_
