...
**Боковое меню Telegram — только 3 команды.** Пользователю видны `/ask`, `/add`, `/addtask` — всё
остальное скрыто из меню (но не удалено, обработчики живы) — решение владельца, канон
[decisions/2026-09-09-bot-menu-three-commands.md](decisions/2026-09-09-bot-menu-three-commands.md).
Единственный источник списка — константа `VISIBLE_BOT_COMMANDS` в `index.ts`, регистрируется через
`setMyCommands` и триггером `{setup_commands:true}`, и автоматически на каждый `/start`.
