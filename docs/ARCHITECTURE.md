...
| `projects` | Проекты Project Space (Рой) | `id` (uuid PK), `group_id` (FK → `workspaces.id`), `name`, `color`, `emoji`, `created_by` (bigint, telegram_id, nullable), `created_at`. Задачи привязываются через `tasks.project_id`/`project_linked` (см. выше). Удаление проекта — `_shared/tasks/projects.ts` `deleteProject()`: FK `ON DELETE SET NULL` обнуляет `project_id` у задач, затем явный апдейт сбрасывает их `project_linked=false` (FK линию не трогает). Миграция `20260801120000` |
...
| `POST` | `/tasks` | Создать (`assignee_telegram_id` → имя); поля Роя: `is_private` (→`owner_id`), `start_date`, `sprint_id`, `tags`, `timeline_position`, `project_id`/`project_linked` (Project Space: `project_id` проверяется на принадлежность воркспейсу → 400 иначе); валидация `start_date<=due_date` и принадлежности спринта воркспейсу; `confirmed=true` |
| `PATCH` | `/tasks/:id` | Частичный апдейт. Приватную чужую → 404, мутация приватной не владельцем → 403. Поддержаны новые поля + смена `is_private`, привязка к спринту/проекту. `project_id` проверяется на принадлежность воркспейсу (400 иначе); `project_id:null` (открепление) сбрасывает `project_linked=false`; `project_linked:true` без (эффективного) `project_id` → 400. `label_ids` (смарт-метки) принимаются **только** для своей личной задачи (`is_private && owner_id === вызывающий`) и только если все id — метки вызывающего, иначе **400** |
...
| `POST` | `/projects` | Создать проект `{ name, color?, emoji? }`; `created_by` = вызывающий |
| `PATCH` | `/projects/:id` | Обновить `{ name?, color?, emoji? }` своего воркспейса → 404, если чужой/не найден |
