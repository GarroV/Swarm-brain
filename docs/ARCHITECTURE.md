...
только в конце, поэтому встреча со свежим (≤ `AGENT_STALE_MIN`) heartbeat агента по её `identity_key`
(`service_agents.last_meeting_key`) призраком не считается. Критерий — одна функция
...
новой колонки или таблицы — решено D018 (`meetings.agent_last_seen_at`/`agent_last_recording`, задача
T151). Тот же разрыв у сторожа призраков: при двух встречах сразу вторая может пометиться `failed`
посреди записи; T151 переключит `botKeepsMeetingAlive` на поле встречи.
