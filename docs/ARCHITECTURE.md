...
| Валидация | `parseSpeakerTimeline` на границе, **до любых записей**. Отвергает 400-й: не-JSON, не массив, не объект в элементе, `name` не строка / пустое / длиннее 120 символов, `start`/`end` не конечные числа, `start < 0`, `end <= start`, больше 5000 интервалов. Текст ошибки несёт индекс: `speakers[3]: end must be greater than start` |
| Чистка имени | Управляющие символы и переводы строк → пробел, пробелы схлопываются. Имя задаёт посторонний участник встречи, а стенограмма построчна (`метка: текст`) — перевод строки в имени ломает разметку и открывает инъекцию в промпт тезисов |
