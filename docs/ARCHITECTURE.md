...
| `DEFAULT_MEETING_LANG` | meeting-ingest, meeting-process (через `_shared/meeting-processor.ts`) | нет, дефолт `ru` | Язык-фолбэк последней инстанции (ISO-639-1) для пина микрофона, когда ни sys-якорь, ни большинство по всем частям язык не дали (офлайн-встреча без sys-дорожки). Пин — мягкий хинт Whisper, не перевод |
