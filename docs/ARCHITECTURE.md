...
| `meeting-context` | HTTP POST (desktop-agent) | «Что было в прошлый раз» для панели заметок рекордера ([#226](https://github.com/GarroV/Swarm-brain/issues/226)). Agent-токен (`smcp_`) → страна созвена по названию встречи (`_shared/market-suggest.ts`, тот же приоритет сигналов, что на вычитке; второй сигнал — общий рынок участников из `user_profiles`) → последняя ОПУБЛИКОВАННАЯ встреча этой страны (`entries`, `entry_type='meeting'`, `metadata.confirmed='true'`, `countries @> [страна]`) → превью тезисов + задачи. Превью нарезает сервер (`_shared/meeting-context.ts`): заголовки разделов, первые 3 пункта, счётчик, полный текст с потолком 20 000 символов и флагом `truncated` (тезисы на проде в среднем 7 КБ, максимум 24 КБ). Задачи — сначала привязанные к той встрече, затем добивка по стране (`mergeContextTasks`): у последней встречи задач обычно НЕТ (на проде BG — ноль), а по стране есть (RS 15, SI 13, BG 3). Скрыты `done`/`cancelled`/`pending`/`backlog` и неподтверждённые: 32 живых `pending` — это невидимый в вебе импорт из #208. Приватное только владельцу (`is_private=false OR owner_id=<смотрящий>`), воркспейс по `group_id`. LLM НЕТ: тезисы и задачи уже в базе. Деплой: `--no-verify-jwt` |
