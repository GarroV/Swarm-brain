...
| `meeting-current` | HTTP GET (desktop-agent) | Swarm Meetings: «какая встреча идёт сейчас» для рекордера. Обмен `refresh`→`access` и запрос событий — общий `_shared/google-calendar.ts` (его же зовёт `swarm-api /calendar/today`, issue #218): своей копии здесь больше нет. Agent-токен (`smcp_`) → `telegram_id` → `refresh_token` из `user_integrations(service='google_calendar')` → Google Calendar API (события now−2мин…now+`LOOKAHEAD_MIN`; **`LOOKAHEAD_MIN = 5` мин** — за сколько до начала встреча считается предстоящей и рекордер предлагает запись; порог задаёт ТОЛЬКО сервер, у рекордера своего нет. Было 10, снижено 04.09.2026 по просьбе владельца) → идущее событие + идентичность для claim. Рекордеру не нужен локальный доступ к календарю. В ответе есть `join_url` — ссылка «зайти в звонок» (`conferenceData.entryPoints[video]` → `hangoutLink` → https-ссылка из места проведения; **только https**, иначе `null`) для кнопки «Подключиться» в капсуле рекордера, `join-link.ts` |
| `meeting-context` | HTTP POST (desktop-agent) | «Что было в прошлый раз» для панели заметок рекордера ([#226](https://github.com/GarroV/Swarm-brain/issues/226)). Agent-токен (`smcp_`) → страна созвона **ТОЛЬКО по явному указанию в названии** встречи (`_shared/market-suggest.ts`, сигнал `title`; рынки участников из профилей НЕ используются — требование владельца «максимальное совпадение», догадка тянет тезисы чужой страны) → последняя ОПУБЛИКОВАННАЯ встреча этой страны (`entries`, `entry_type='meeting'`, `metadata.confirmed='true'`, `countries @> [страна]`) → превью её тезисов. **Задачи — ВТОРАЯ, независимая выборка: по самой стране** (`tasks.country`), а НЕ по найденной встрече: «задачи и тезисы никак и не должны соприкасаться, это разные вещи» (владелец 03.09.2026). Превью нарезает сервер (`_shared/meeting-context.ts`): заголовки разделов, первые 3 пункта, счётчик, полный текст с потолком 20 000 символов и флагом `truncated` (тезисы на проде в среднем 7 КБ, максимум 24 КБ). Задач по стране на проде: RS 15, SI 13, BG 3 (а вот привязанных к последней встрече обычно нет вовсе — ещё одна причина не связывать секции). Скрыты `done`/`cancelled`/`pending`/`backlog` и неподтверждённые: 32 живых `pending` — это невидимый в вебе импорт из #208. Приватное только владельцу (`is_private=false OR owner_id=<смотрящий>`), воркспейс по `group_id`. LLM НЕТ: тезисы и задачи уже в базе. **Покрытие:** страна распознаётся у 26 из 120 последних встреч (22 %, замер 03.09.2026) — RS 6, BG 5, SI 3, HU 3, MD/HR 2; города и регионы детектор не знает («Созвон про Марибор», «Catalonia // Weekly sync» проходят мимо). Деплой: `--no-verify-jwt` |
| `meeting-heartbeat` | HTTP POST (desktop-agent) | Heartbeat рекордера: раз в ~15 мин + при старте/смене статуса записи (`recording:true/false`) пишет `allowed_users.recorder_last_{seen,recording,version}`. С 04.09.2026 несёт ещё **присутствие** для `ON AIR` в панели «Встречи сегодня» — `on_call` (идёт реальный созвон: вход микрофона держит другое приложение) и `meeting_key` (`<uid>:<дата>` встречи) → `recorder_last_{on_call,meeting_key}`; ключ хранится только пока звонок или запись идут, иначе `NULL` — иначе он завис бы после созвона. Пока звонок идёт, рекордер шлёт keep-alive раз в 2 мин (`RecorderKit.PresenceBeacon`), панель считает присутствие живым 5 мин. Читается watchdog'ом `checkRecorderHealth` (swarm-bot, из `sweepStuckMeetings`-cron) для 2 сигналов: **оборванная запись** (`recording=true` + молчит >20 мин = краш во время записи → алерт записавшему) и **истечение токена** (<7 дней → алерт `/recordertoken`). Заменил ложный Read.ai-watchdog «встречи не поступают». Auth — персональный токен |
| `google-oauth` | HTTP redirect (OAuth) | Серверная Google Calendar-интеграция для рекордера (как Granola/Read.ai). `/start` редиректит на consent Google (scope `calendar.events.readonly`), `/callback` меняет код на токены и кладёт `refresh_token` в `user_integrations(service='google_calendar')`. **`/link`** (2026-08-28) — тот же приём для привязки, добытой на экране ВХОДА: `POST {telegram_id, refresh_token, sig}` с подписью `HMAC(telegram_id|refresh)` на `WEB_JWT_SECRET`, кладёт refresh в ту же таблицу (вызывает CF Pages, у которого нет `SERVICE_ROLE`). State — подписанный JWT с `telegram_id` (выдаёт `swarm-api` `/google/connect-url`). Секреты `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` |
...
│   ├── daily-report.ts      # /report (админ) + cron daily_report_cron — чистое ядро: yesterdayWindow/aggregateActivity/formatReport/buildDailyReport (ретрай + развязка двух запросов)
│   ├── daily-report-send.ts # sendDailyReport() — I/O поверх ядра: запросы entries/meetings (ошибка → исключение для ретрая) + отправка ADMIN_USER_ID
...
└── lib/
    ...
    ├── openai.ts            # chatComplete(), getEmbedding() — поверх общего withRetry
    ├── retry.ts             # withRetry(fn, {attempts, baseDelayMs, sleep}) — общий повтор разовых сбоев (504 шлюза, сеть); sleep подменяется в тестах
...
| `task_history` | **Журнал изменений задач** (issue #286): по строке на каждое изменившееся поле | `id`, `task_id` (FK → `tasks`, ON DELETE CASCADE), `field` (**любое поле задачи**: `status`, `due_date`, `assignee`, `project`, `sprint`, `priority`, `title`, `description`, `country`, … — решение владельца 09.09.2026 «уметь всё что угодно отмечать»; blacklist вместо whitelist, поэтому новое поле попадает в журнал само. Мимо журнала идут только служебное и шум: `id`, `group_id`, `created_at`/`updated_at`, `completed_at` (производное от статуса), `assignee_telegram_ids` (дубль), `tree_x`/`tree_y`/`timeline_position` (перетаскивание мышью), `reminded_at`), `old_value`, `new_value` (значения обрезаются до 200 символов — журнал не хранилище описаний), `changed_by_telegram_id`, `group_id`, `changed_by` (legacy `text` **NOT NULL** — писался ботом именем пользователя; веб и MCP имени не знают, поэтому `actorName()` кладёт туда имя → иначе telegram_id строкой → иначе `system`. **`null` там ронял вставку на NOT NULL, а `updateTask` глотал ошибку в `console.error` — журнал молча не писался вовсе из веба и MCP: issue #287, поймано на проде 09.09.2026, 4 смены статуса после раскатки и ноль строк в журнале; фикс раскатан в ту же ночь и проверен на проде — две смены статуса через MCP дали две строки с `changed_by='744230399'`**), `old_status`/`new_status` (дублируют значения при `field='status'`), `note`, `created_at`; индексы `(task_id, created_at desc)`, `(created_at desc)`. **Единственный писатель — `updateTask` в `_shared/tasks/db.ts`** (веб, бот и MCP ходят через неё; прежняя вставка из бота убрана, иначе события из бота удваивались). Решение, какие строки породит патч — чистая функция `_shared/tasks/history.ts` (`historyRowsFor`, 9 тестов). ⚠️ **Данные копятся только с раскатки этого журнала**: до 09.09.2026 историю писал лишь бот и лишь на перекате — на проде лежали 2 строки на 2 задачи, прошлые перемещения восстановить нечем |
...
| `task_comments` | Комментарии-апдейты к задаче (веб + MCP) | `task_id` (FK → `tasks`, ON DELETE CASCADE, индекс `idx_task_comments_task_id`), `content`, `added_by_telegram_id` (bigint; имя резолвится на чтении), `added_by` (legacy, nullable), `created_at` (индекс `idx_task_comments_created_at` DESC — под `get_recent_comments`, #276) |
...
1. **Гейт по `identity_key` — только для СРАВНИМЫХ ключей** (issue #164, 2026-08-28). `identity_key` идентифицирует НЕ встречу, а то, **как её увидел конкретный клиент**, и живёт в трёх пространствах имён: `<google-event-id>:<дата>` (kind `calendar`), `kontur:`/`meet:<комната>` (kind `room`), `granola:<note_id>` (kind `external`), `manual:…`. Первые два — **общие** у всех участников (два рекордера одного события дают идентичный ключ), последние два — **персональные** (у каждого свой). Поэтому гейт применяется, только когда оба ключа из одной ОБЩЕЙ схемы (`comparableKeys`): **равны → дубль**, **различаются → РАЗНЫЕ встречи, склеивать нельзя** (регрессия IMF BD 23.07: 4 разные встречи одного дня с идентичным составом слиплись в запись `da4cef3f` именно из-за отсутствия гейта). Ключи из разных пространств или из персональной схемы гейтом НЕ разводятся — падаем в эвристику. ⚠️ До 2026-08-28 гейт срабатывал на ЛЮБОЙ паре непустых ключей и **отключал кросс-источниковый дедуп целиком**: `granola:not_X` и `<event>@google.com:дата` не совпадут никогда, функция уходила в `continue` до всякой эвристики. Так одна встреча «IT+BD» 26.08 легла в базу тремя записями (рекордер-календарь + рекордер-Контур.Толк + Granola), а всего на проде накопилось 13 таких пар.
2. **Эвристика** (когда ключи несравнимы или их нет). Та же `entry_date` + любой из трёх сигналов, все калиброваны сухим прогоном по всем 242 записям-встречам прода (13 склеек, **ни одного** ложного срабатывания):
   - **состав** — СИЛЬНОЕ пересечение участников (≥2 человек И ≥ половины меньшего списка) + время **±5 мин**, если известно у обоих. Сильное пересечение нужно, потому что разные встречи делят 1–2 общих человек (реальный отсечённый ложный дубль: 1-1 «Maria/Aleksandra» 08:00 ⨯ большая «CVM IMF» 08:15). Точный хэш состава НЕ годится — у разных участников списки различаются;
   - **идентичное название** (нормализация: только буквы/цифры, нижний регистр) — единственный сигнал для Granola: участников она не отдаёт вовсе. Окна времени у него намеренно нет: Granola подключается к созвону позже начала (на проде разрыв доходил до 25 мин), а одноимённых РАЗНЫХ встреч в один день на проде нет ни одной. Дефолтные и короткие названия («Встреча», «1-1», `normTitle` < 4 символов) сигналом не считаются. Префикс заголовка НЕ используется сознательно: «Dodo Pizza Bulgaria» и «Dodo Pizza Hungary» — разные встречи;
   - **участие публикующего** (`viewerEmail` ∈ `metadata.attendees` кандидата) + время **±10 мин** — для записи из комнаты: у неё нет ни названия, ни участников. Живой случай 26.08: у коллеги события в календаре не было (пришла по ссылке), рекордер опознал только комнату Контур.Толк — но её e-mail есть в attendees календарной записи. Применяется ТОЛЬКО к безымянной входящей и только в узком окне: иначе склеили бы встречу, на которую человек лишь приглашён, с той, где он реально был.

   Время кандидата берётся из `meetings.started_at` (доп. запрос по `metadata.meeting_id`, не-uuid отфильтрованы) и лишь затем из `content`: у записей рекордера строки «Дата: …, HH:MM» нет вовсе. ⚠️ `toMinutes` до 2026-08-28 разбирал формат PostgREST неверно — на `"2026-08-26T12:01:36+00:00"` regex по «HH:MM» хватал «01:36» (минуты с секундами) и возвращал 96 вместо 721, поэтому окна времени сравнивали мусор молча (issue #172).

   Кандидаты берутся по `entry_date` (индекс `idx_entries_date`). Что дедуп НЕ ловит сознательно: источники с РАЗНЫМИ названиями и без участников («IT+BD» ⨯ «Встреча АЙТИ и БД 15.07») и приватные записи РАЗНЫХ владельцев (склейка означала бы показ чужого личного).
...
| Рекордер публикация | `swarm-api` `POST /agent-meetings/:id/publish` | привязать `meeting` к существующей записи, **арбитраж по полноте** (ниже) и вернуть запись с флагами **`duplicate: true`**, `replaced`, `arbitration` (только если запись видима публикующему — публичная/его личная; чужие приватные игнорируем). Флаги обязательны: веб говорит, чья версия теперь в базе, вместо прежнего «Черновик опубликован» (issue #170) |
| Read.ai вебхук | `read-ai-webhook` | skip ДО дорогих LLM-вызовов и `extractAndSaveTasks` (иначе задачи-сироты) |

**Арбитраж версий на публикации — «в базу идёт САМАЯ ПОЛНАЯ встреча»** (`_shared/meeting-fullness.ts`, issue #176, решение владельца 2026-08-28 — [decisions](decisions/2026-08-28-one-meeting-one-record.md)). Дедуп находит, что встреча уже в базе; кто её опубликовал раньше — не аргумент. Это тот же канон, что у перехвата в claim: правило «кто первый» стоило проекту записи на 2ч26м (#23/#24).

- **Метрика — объём РАСПОЗНАННОГО** (сумма длин `transcript.segments[].text`), не длительность аудио. На живом случае 26.08 длительности почти равны (1920 с против 1980 с — порог claim ×1.5 не берётся), а распознано вдвое разное: **43 228 против 18 212 символов (×2.37)**, 1097 против 469 сегментов. Разошлись версии не длиной звонка, а потерей звука собеседника у одного из рекордеров (#10) — длительность к этому слепа.
- **Порог ×1.5** (`FULLNESS_MIN_RATIO`): близкие версии не гоняют перезапись.
- Транскрипт есть у входящей и отсутствует у опубликованной (Granola) → входящая полнее по определению. Обратный случай замены не даёт.
- **Правленное человеком не перезаписывается** — ни в базе (`published_edited`), ни у входящей (`incoming_edited`): та же защита `notes_edited_at`, что у перехвата в claim.
- Замена меняет только СОДЕРЖИМОЕ записи (`content`/`summary`/`embedding`) — **id сохраняется**, ссылки и задачи не рвутся. Прежние тезисы остаются в `draft_notes_md` своей строки `meetings`, факт замены пишется в `metadata.superseded` (когда, кем, причина, прежний `meeting_id`).
- `GET /agent-meetings/:id` отдаёт **`in_base_duplicate`** — та же встреча уже в базе. Веб показывает плашку ДО публикации («эта встреча уже в базе, запись общая; в базе останется самая полная версия», а если человек правил тезисы — «твои правки не заменят её автоматически»), а не сообщает об этом тостом после нажатия «Опубликовать».

⚠️ Корень расхождения версий лечится не здесь: рекордер отправляет только ОДИН `identity_key` (календарь приоритетнее комнаты), поэтому участник с событием в календаре и участник, пришедший по ссылке, не опознаются как одна встреча и **транскрибируют оба**. Направление — все ключи + склейка на claim, issue #168.
...
Тесты — `_shared/meeting-dedup.test.ts` (26 штук: ложный дубль по составу, `identity_key`-гейт на сравнимых ключах — кейс IMF BD 23.07, ключи из разных пространств, granola↔granola с разными `note_id`, сигнал названия и его отказ на «Dodo Pizza Bulgaria/Hungary» и на дефолтных названиях, сигнал участия с окном времени, формат времени PostgREST). Накопленные ДО внедрения дубли (7 пар) объединены в 2026-08. ⚠️ Дубли, накопленные с тех пор (13 пар, 2026-06…08), **в базе остались** — владелец 2026-08-28 решил данные не трогать, починен только код.
...
Все участники записывают аудио → meeting-claim (на СТОПЕ записи, до загрузки):
  ...
  ⚠️ ОДНА ВСТРЕЧА, ДАЖЕ ЕСЛИ КЛЮЧИ РАЗНЫЕ (issue #168, 2026-08-28): рекордер шлёт ОДИН ключ и
     календарь приоритетнее комнаты, поэтому участник с событием в календаре присылал
     `<event>:<дата>`, а пришедший по ссылке — `kontur:<room>`; уникальный индекс такую пару не
     видит и claim открывал ВТОРУЮ встречу (два аудио, ДВЕ транскрибации, два черновика).
     Теперь перед вставкой claim ищет встречу того же воркспейса с началом ±10 мин и
     подтверждением по людям (`_shared/meeting-roster.ts`): записавший одной стороны есть в
     attendees другой ЛИБО сильное пересечение составов (≥2 и ≥половины меньшего). Одного
     времени недостаточно — в одну минуту идут разные созвоны. Найдено → присоединяемся к
     существующей строке (тот же арбитраж, что при совпадении ключа), новая не создаётся;
     гонка страхуется повторным поиском после вставки (свою пустую строку убираем).
     Калибровка проверена сухим прогоном по всем 152 прод-встречам: 2 склейки, обе настоящие
     (26.08 «IT+BD» ⨯ «Встреча» из Контур.Толк; 20.08 календарная ⨯ ручная запись коллеги —
     ручные пока НЕ склеиваются, ветка `manual` намеренно всегда создаёт новую встречу).
  ⚠️ КОМНАТНЫЙ КЛЮЧ СУЖЕН ДО ДНЯ (issue #181): `kontur:<room>` / `meet:<code>` у регулярной
     встречи постоянны (у повторяющегося события Google Meet одна ссылка на серию), а индекс
     глобальный — второй созвон в той же комнате находил старую строку и получал `defer`, то
     есть НЕ записывался вовсе. Сервер добавляет `:<YYYY-MM-DD>` (`scopeRoomKey`) — не клиент,
     потому что у команды стоят разные сборки рекордера.
...
```
**Баннер встречи (капсула до записи).** Когда рекордер увидел встречу или звонок, `RecorderWidget` показывает не узкую вертикальную капсулу, а **баннер**: янтарная полоска · название встречи · время слота с обратным счётом (`09:00 – 10:00 · через 5 мин`) · кнопки **«Подключиться»** (открывает звонок И включает запись) и **«Записать»** · ✕ в правом верхнем углу. Ссылки нет → кнопки «Подключиться» тоже нет, а «Записать» становится главной (залитой). Текст баннера даёт `RecorderKit/MeetingNotice`. **Это единственная поверхность предложения записать**: штатное уведомление macOS о встрече (`notifyMeeting`) и о звонке (`postCallNotification`) убраны 07.09.2026 — они дублировали капсулу и висели в Центре уведомлений после того, как повод исчез ([decisions/2026-09-07](decisions/2026-09-07-one-surface-for-meeting-prompt.md), [#263](https://github.com/GarroV/Swarm-brain/issues/263)). Штатными баннерами остаются только ИНФОРМАЦИОННЫЕ сообщения рекордера («отправлено», «звонок завершён», «нужен новый токен», «системный звук не пишется», «запись отложена/подобрана») — у них второй поверхности нет; клик по ним ничего не запускает. Ссылка приезжает с сервера в `join_url` (`meeting-current`), пропускается только `https` (`RecorderKit/JoinLink` — второй барьер после серверного `join-link.ts`). Место капсулы считает `RecorderKit/WidgetPlacement`: перетащил — запомнили (`UserDefaults`, `widget.origin`), не таскал — правый край ниже полосы управления. Решение владельца 02.09.2026 — [decisions/2026-09-02-pill-and-join-button.md](decisions/2026-09-02-pill-and-join-button.md) ([#193](https://github.com/GarroV/Swarm-brain/issues/193), [#197](https://github.com/GarroV/Swarm-brain/issues/197)). Во время записи баннер уступает место `LiveNotesPanel` (ниже).

**Правая колонка главной: «Встречи сегодня» сверху, записанные снизу** ([#218](https://github.com/GarroV/Swarm-brain/issues/218), порядок задан владельцем 03.09.2026). Верхняя панель (`dash/MeetingsToday.tsx`) читает `GET /calendar/today` в `swarm-api`: серверная OAuth-интеграция → события суток пользователя (границы считает `_shared/meetings-today.ts` по присланному смещению пояса — у edge-функции своего пояса нет) → отбор (отменённые, «свободен»/OOO и события на весь день не встречи) → сортировка по времени. Идущая встреча подсвечена акцентом и подписана «идёт», прошедшие приглушены, кнопка **«Подключиться»** есть только при https-ссылке и только у не закончившихся. Причины пустоты РАЗЛИЧАЮТСЯ: `not_connected`/`token_expired` → кнопка «Подключить календарь»/«Переподключить» (решение владельца: «там же можно сделать кнопку»), `calendar_error` → «Календарь не ответил», иначе «Сегодня встреч нет». Опрос раз в 5 минут и при возврате на вкладку — «идёт сейчас» иначе врёт до перезагрузки. **`token_expired` — ТОЛЬКО реально мёртвый refresh_token** (issue #302, 2026-09-10): `_shared/google-calendar.ts` `accessToken()` различает ответ Google `invalid_grant`/`invalid_client` (`isDeadGrantError`, токен правда отозван/протух) от временной запинки (429/5xx/сеть) — раньше ЛЮБАЯ осечка обмена refresh→access трактовалась как «мёртвый токен», и рекордер/панель звали переподключаться на пустом месте при обычном чихе Google API; временная запинка теперь идёт в `calendar_error`/`calendar_api_error` без призыва переподключаться. Каждая осечка логируется (код + текст ошибки Google, без самого токена) — раньше отказ был молчаливым и постфактум не диагностировался.

**`ON AIR` / `REC` — где сидит сам человек** (решение владельца 04.09.2026, канон — [decisions/2026-09-04-on-air-v-panele-vstrech.md](decisions/2026-09-04-on-air-v-panele-vstrech.md)). Предлагать «Подключиться» у встречи, в которой человек уже сидит, — значит не понимать, что происходит. Поэтому строка несёт два НЕЗАВИСИМЫХ факта: `on_call` («ты в звонке») и `recording` («рекордер его пишет») — в звонке можно сидеть без записи. Источник — heartbeat рекордера: `CallDetector.realCallActive()` видит, что вход микрофона держит другое приложение, ключ встречи приходит из `meeting-current`. Флаг зажигается при совпадении ключа `<uid>:<дата>` и heartbeat не старше 5 минут; границу по времени слота сервер НЕ проверяет намеренно (её задаёт окно ±10 мин у `meeting-current`) — иначе затянувшийся созвон потух бы, пока люди ещё говорят. В строке при этом: чип `ON AIR` (+ `REC`), счётчик участников скрывается (место в мета-строке кончается, а в звонке людей и так видно), кнопка становится тихой «Вернуться / Back» — вкладку звонка легко закрыть случайно, и путь назад нужен. Нажатие «Подключиться» запоминается в сессии вкладки до конца слота (`lib/joinedCalls.ts`) и переключает строку МГНОВЕННО, не дожидаясь heartbeat; серверный флаг главнее — он знает и про звонок, в который зашли не отсюда. Границы сигнала: звонок с телефона или другой машины не виден, спящий мак не виден, а любое приложение с занятым микрофоном (голосовое сообщение, диктофон) даёт ложный плюс — потому и требуется совпадение ключа. Нижняя панель (`MeetingsApprove`) показывает **только вычитку**: ярус «Недавние» (опубликованные) убран, а оба источника очереди — неподтверждённые `entries` и черновики рекордера (`awaiting_review`) — теперь в одном ярусе, поэтому бейдж и строки больше не расходятся (issue #220: было «27 на согласовании» при 3 видимых). У панели «Встречи сегодня» нет своего экрана, поэтому `DashBlock.onHead` стал необязательным: без него шапка — заголовок, а не кнопка, обещающая переход.

**Блок «С прошлого раза» в панели заметок** ([#226](https://github.com/GarroV/Swarm-brain/issues/226), решения владельца 03.09.2026: «эта сторона = эта страна», «тезисы последней встречи», «задачи показываем связанные с этой встречей»). На старте записи рекордер фоново дёргает `meeting-context` и рисует над списком пометок две раскрывающиеся секции: **Тезисы** (дата · «12 пунктов», свёрнуто — 3 первых пункта, раскрыто — полный текст) и **Задачи** (до 2 строк свёрнуто, клик по строке уводит в веб по `?task=<id>`). Блок живёт ВНУТРИ прокрутки над пометками, поэтому поле «пометка на полях…» остаётся внизу окна и не уезжает при раскрытии. Запрос необязательный: не ответил — блока просто нет, запись не ждёт сети и не падает. Секции независимы: тезисы приходят от последней встречи, задачи — от стороны созвона, и одно не зависит от другого. ⚠️ **Барьер:** контекст живёт только во вью и НИКОГДА не попадает в буфер пометок — тот пополняется единственным местом, `addFromField()` (нажатие человека), и ровно он уходит на сервер в `flush()`. Требование владельца 03.09.2026: отображаемые тезисы и задачи не должны смешаться с тем, что человек пишет сам. Проверяется замером в `--selftest-notes` (печатает размер буфера до и после показа контекста).
...
| `TELEGRAM_WEBHOOK_SECRET` | swarm-bot | нет | Секрет вебхука Telegram (#280). Уходит в `setWebhook` как `secret_token`, Telegram присылает его обратно заголовком `X-Telegram-Bot-Api-Secret-Token`. Пустое значение Telegram трактует как «снять секрет», поэтому в запрос подставляется только когда задан |
| `TELEGRAM_WEBHOOK_ENFORCE` | swarm-bot | нет (`1` = включено) | Включает отбивку апдейтов без верного `secret_token` (401 до разбора тела, сверка за постоянное время). **Намеренно отдельно от значения:** включить проверку раньше, чем Telegram узнает секрет через `setWebhook`, значит оборвать бота живым людям. Порядок: задать `TELEGRAM_WEBHOOK_SECRET` → дёрнуть `set_webhook` → убедиться, что апдейты идут → `TELEGRAM_WEBHOOK_ENFORCE=1` |
...
3. **Браузер — Google Sign-In (ОСНОВНОЙ, 2026-07-29)** — кнопка «Sign in with Google» на `/login`.
   - CF Pages `/api/auth/google/{start,callback}` (переиспользует OAuth-приложение календаря; `GOOGLE_CLIENT_ID/SECRET` в CF-env, нормализуются от случайного `http://`): scope **`openid email profile` + `calendar.events.readonly`** (канон набора — `_shared/google-scopes.ts` `LOGIN_SCOPES`), `access_type=offline`, `include_granted_scopes=true`, `hd=dodobrands.io` + **серверная сверка домена** verified email = `dodobrands.io`. state — HMAC(next) на `WEB_JWT_SECRET`.
   ...
   - **Календарь привязывается ВХОДОМ (2026-08-28)** — «вошёл через Google → календарь подключён», как у Granola (канон [decisions/2026-08-28-google-login-connects-calendar.md](decisions/2026-08-28-google-login-connects-calendar.md)). Google показывает granular-экран, где галочку календаря можно **снять**; тогда `scope` в ответе на обмен кода её не содержит — календарь просто не привязывается, **вход при этом проходит** (определяется verified email, а не набором прав). Проверка выданных прав — `hasCalendarScope` (`_shared/google-scopes.ts`, зеркало в `miniapp/functions/_lib/google-name.ts`, эквивалентность под тестом), сравнение целыми элементами: `calendar.events`/`calendar.readonly` — ДРУГИЕ права.
     `refresh_token` попадает к CF Pages, а `SERVICE_ROLE` там не держим — поэтому Pages отдаёт его **`google-oauth/link`** (POST, server-to-server, подпись `HMAC(telegram_id|refresh)` на `WEB_JWT_SECRET`), и уже он пишет `user_integrations`. Google отдаёт `refresh_token` только при первом согласии на календарь: у кого привязка уже есть, поле придёт пустым — перетирать нечего. Сбой привязки не валит вход: человек увидит «не подключён» в Настройках и нажмёт кнопку (**отдельный поток `google-oauth/start` остаётся** ремонтным путём).
...
| `GET` | `/sprint-cycles` | **Спринты** воркспейса (все участники). Модуль `swarm-api/sprint-cycles.ts` |
| `POST` | `/sprint-cycles` | Создать спринт (`name`, `start_date`, `end_date`) — **только admin** |
| `GET` | `/sprint-cycles/:id` | Спринт + состав (`items`: живой у идущего, из клона у принятого) |
| `PATCH`/`DELETE` | `/sprint-cycles/:id` | Правка (`name`, даты, `summary`) и удаление — **только admin**; принятый спринт не удаляется |
| `POST` | `/sprint-cycles/:id/start` | `draft` → `active`: состав фиксируется как ПЛАН (`in_plan=true`) — **только admin** |
| `POST` | `/sprint-cycles/:id/accept` | Приёмка: клон в `frozen_*`, расчёт `stats`, перенос незакрытых в `next_cycle_id` — **только admin**. Перенос в уже принятый спринт запрещён |
| `POST` | `/sprint-cycles/:id/tasks` | Добавить задачи (`task_ids`) — любой участник. Молча отсеиваются чужой воркспейс, приватные и уже добавленные; статус `backlog` → `open` (в спринте такой колонки нет) |
| `DELETE` | `/sprint-cycles/:id/tasks/:taskId` | Убрать задачу из спринта (пока не принят) |
| `GET` | `/sprints` | ⚠️ ВКЛАДКИ доски проектов, не спринты (все участники) |
...
| `get_tasks` | Список задач с фильтрами (в т.ч. `label` — имя личной смарт-метки, `status` включает `backlog`). **Неизвестный `project` → отказ со списком доступных имён, а не молчаливый игнор фильтра** (issue #199). Неподтверждённая задача помечена `⏳ на проверке (в вебе не видна)` — иначе агент принимает её за задачу на доске (issue #201). **В строке печатается полный `id` задачи** (issue #275) — без него `get_task_comments`/`add_task_comment`/`update_task` нечем вызвать, а резолва по имени или префиксу нет |
...
| `get_task_comments` | Комментарии-апдейты к задаче по её ID (если задача доступна вызывающему). ID — полный uuid из строки `get_tasks` |
| `get_recent_comments` | **Свежие комментарии по ВСЕМ доступным задачам одной выдачей** (issue #276) — вход для дайджеста «что нового». `since` (ISO, по умолчанию сутки; неразобранное значение → отказ, не тихий дефолт), `limit` (50, максимум 200). Сгруппировано по задаче, у каждой печатается `id`. Видимость — тем же `canViewTask` + воркспейс, что у поштучного чтения; читаем с 4-кратным запасом к `limit`, иначе чужой приватный тред съедал бы выдачу. Упёрлись в лимит → выдача **говорит, что обрезана** |
| `add_task_comment` | Добавить комментарий-апдейт к задаче по её ID от лица вызывающего |
| `get_task_stats` | **Статистика по задачам за период** (issue #286): создано/закрыто **с раскладкой по дням (окно ≤45 дней) или месяцам**, lead time (среднее и медиана), **время ожидания начала работы и время в работе — по журналу, с печатью `basis`** (по скольким задачам считали), доля закрытых в срок, открыто/в работе/просрочено, кто сколько закрыл, возраст самой старой незакрытой. Выборка идёт с `confirmed: true` — иначе `listTasks` применяет дефолт «не показывать done/cancelled» и «закрыто за период» считалось нулём; и с `isAdmin` — админ видит все задачи, включая личные ([решение](decisions/2026-08-21-admin-visibility.md)). Упёрлись в потолок выборки → выдача говорит, что посчитано не по всем. Окно — `period` (day/week/month/quarter/year) или точный `since`; фильтры `assignee`/`project`/`country`. Неизвестный `period` и неразобранный `since` → отказ, а не тихий дефолт. Расчёты — чистые функции `_shared/tasks/analytics.ts` (`computeTaskStats`, 8 тестов), формат — `swarm-mcp/tasks/analytics-format.ts` (10 тестов). Пустой lead time печатается словами, а не нулём |
| `get_task_history` | История одной задачи из журнала: что, когда и кем менялось (статус, срок, исполнитель, проект, спринт, приоритет). Старые строки без `field` читаются как смена статуса |
| `get_recent_task_changes` | **«Где, когда, куда передвинули» одной выдачей** — все изменения по доступным задачам за период, сгруппированы по задаче с `id`. Видимость — `canViewTask` + воркспейс (журнал приватности не знает, поэтому спрашиваем задачи); читаем с 4-кратным запасом к `limit`; упёрлись в лимит → выдача говорит, что обрезана |
