...
| `get_sprint_spaces` · `create_sprint_space` · `rename_sprint_space` · `archive_sprint_space` · `get_sprints` · `get_sprint` · `create_sprint` · `update_sprint` · `start_sprint` · `accept_sprint` · `delete_sprint` · `add_sprint_tasks` · `remove_sprint_task` · `mark_sprint_task` | **Спринты целиком из MCP** (issue #480, владелец 24.09.2026: «важно чтобы мсп умел рулить всем функционалом спринтов»). Модуль `swarm-mcp/tasks/sprints.ts` (инструменты, таблица `SPRINT_TOOLS` — диспетчер в `index.ts` один на всех) + `sprints-format.ts` (тексты, под тестами). Логика — те же `_shared/tasks/sprint-*.ts`, что у веба; права — как в `swarm-api/sprint-cycles.ts` и `/sprints`: всё любому участнику, кроме `delete_sprint`, `rename_sprint_space`, `archive_sprint_space` (админ: `ADMIN_USER_ID` или `allowed_users.is_admin`). ⚠️ Работает **только с `kind='space'`**: пространство выбирается по имени/id из `listSprints(…, "space")`, вкладку доски «Проекты» не берёт даже по точному id (тест). Состав — глазами спрашивающего (`listItems` с `viewer`): чужая личная задача печатается строкой без названия и без `task_id`; ответ `mark_sprint_task` заголовка не печатает (`updateItem` отдаёт строку не глазами смотрящего) |
