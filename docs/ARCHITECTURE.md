...
| Код | Действие |
|----|---------|
| `help_main` | Вернуться к главному экрану справки |
| `help_<topic>` | Открыть подменю: `recorder` / `granola` / `mcp` / `kb` / `team` (правка сообщения на месте) |

Главный экран `/help` компактный (повседневное + ссылка на веб **Swarm**); детальные настройки спрятаны за inline-кнопки (`handlers/help.ts`: `helpKeyboard()`, `getHelpTopic()`), открываются через `editInlineMessage`. Подменю рекордера включает шаг привязки Google-календаря (Swarm → Настройки).
