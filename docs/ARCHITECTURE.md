...
| `GET` | `/stats/people` | Экран «Статистика»: по каждому участнику воркспейса `{tasks:{open,inProgress,overdue,closed,closedRecent,onTimeRate,onTimeBase,avgCloseDays}, meetings:{published,inReview}, activity:{activeDays,strip[14],lastActiveAt}}` + `activityDays`, `closedWindowDays`, `reviewVisible`, `tasksTruncated`. Считает `_shared/stats/people.ts` (дни по Белграду), выборки — `swarm-api/stats.ts` через общие правила видимости (задачи `listTasksWithTotal`, записи `buildEntriesQuery`). Только числа и даты. `inReview` — **только админу** (иначе `null`), как у `/admin/review-counts`. Активность = правки задач (`task_history`), комментарии, авторство записей, запись встреч (`recorders[].claimed_at`) за 90 дней |
