...
| Что просит агент | Что проверяет сервер | Отказ |
|---|---|---|
| календарную встречу (`identity_kind: calendar`) | событие с этим ключом (`<iCalUID>:<дата>`) есть **в Google-календаре названного человека** — сервер смотрит сам; состав из тела запроса не в счёт, его прислал тот же агент | нет в календаре → 403; календарь не подключён → 403 с подсказкой запустить бота руками; Google не ответил → 503 (сверка не выполнена ≠ пройдена) |
| встать в уже открытую встречу (тот же ключ или склейка по составу) | человек в её составе (`attendees[].email`, регистр не важен) или уже держит её (`claim_owner`) | склейка с чужой встречей не происходит — заводится своя; конфликт точного ключа → 403 |
| ручную или комнатную встречу | ничего сверх воркспейса: состава у них нет, а ручной путь по D015 постоянный | — |

Людей (рекордер, MCP) сверка не касается: их токен и есть их личность. Остаточный риск, принятый D016:
комнатная встреча без состава (`attendees` пустой) открыта агенту любого человека своего воркспейса.
