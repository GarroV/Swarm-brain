...
|---|---|---|---|
| `<iCalUID>:<YYYY-MM-DD>` (как собирает `meeting-current`) | только `calendar` | событие есть **в Google-календаре названного человека**; нет → 403, календарь не подключён → 403 с подсказкой запустить бота руками, Google не ответил → 503 | из события Google, тело запроса не в счёт |
