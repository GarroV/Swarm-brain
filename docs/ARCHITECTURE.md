...
| `meeting-notice` | HTTP POST (бот-участник `scriba`) | **Громкий отказ доходит до человека** (блок `notices`): бот в контейнере токена Telegram не имеет, поэтому приносит сюда причину, по которой запись НЕ идёт, а сервер шлёт её владельцу встречи тем же Telegram, что и `meeting-processor`. Тело: `{kind, title?, attempt?, lang?, detail?}`, девять видов отказа — `door_waiting`, `door_denied`, `captcha`, `no_conference_link`, `no_owner`, `no_audio`, `recording_lost`, `container_died`, `join_failed` (`_shared/notices.ts`; тексты en/ru — `_shared/notice-texts.ts`, по умолчанию en). **Получатель — всегда человек из `resolveActingIdentity` и никогда из тела запроса**, иначе токен бота стал бы рассылкой. **Дверь:** `attempt: 1` → ответ `next_reminder_in_s: 180`; `attempt: 2` → единственный повтор и `should_leave: true`; `attempt: 3` → **409 и ничего не отправлено** (потолок держит сервер). Недоставленное сообщение — **502 с причиной**, а не 200: проглоченный отказ Telegram и есть молчаливый сбой. Прогон всех сценариев отказа — `scripts/scriba-notices-smoke.ts`. Auth — личный токен человека ИЛИ токен служебного агента с `X-On-Behalf-Of`. Деплой: `--no-verify-jwt` |
