...
Схема, правила и атомарность: поля сверки и переноса, один живой спринт на пространство, функция приёмки
одной транзакцией, триггер упоминания удалённой задачи, гранты. Отдельный блок, потому что это
единственное место, где правило держится независимо от того, каким путём пришли данные — а задачи в Swarm
удаляются тремя разными путями.
...
-- {next_cycle_id uuid, carried int, mentions int}
-- отказы: P0001 (спринт не активен, нет прав на пространство), 23505 (гонка приёмки / второй живой)
```
Плюс: поля §5 спеки, `carry_count`, частичный уникальный индекс `uniq_sprint_cycles_live_per_tab`,
триггер `before delete on tasks` → `removed_title`/`removed_project_id`/`removed_at`,
`check (jsonb_typeof(links) = 'array')` через `NOT VALID` + `VALIDATE`,
...
`core` (тест функции приёмки сверяет её результат с `planCarry`). Внешнего — Postgres 17, Supabase CLI.
...
| 1 | миграция аддитивна: только `ADD COLUMN`, индексы, триггер, функция; `supabase db reset` проходит |
| 1 | приёмка атомарна: тест на локальной базе — результат совпадает с `planCarry`, повторный вызов отбит, живой спринт после приёмки ровно один |
| 1 | триггер: удаление задачи через API и прямым `delete` оставляет упоминание; замороженные строки не трогаются |
| 1 | права: `has_function_privilege('anon', 'accept_sprint_cycle(...)', 'execute') = false`, `get_advisors` чист |
| 1 | гранты новым объектам прописаны явно (D006) |
| 1 | спринт владельца удалён отдельной операцией; счёт задач до и после совпал |
