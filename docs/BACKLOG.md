...
**🟢 СДЕЛАНО**
- **Цифры вместо имени исполнителя** («#744230399» на карточке и в miniapp-детали). Причина: имя денормализуется в `tasks.assignees` при создании; если профиля ещё не было — туда падал сырой `telegram_id`. Фикс: `swarm-api` `GET /tasks` и `GET /tasks/:id` перерезолвят имена из актуальных `user_profiles` по `assignee_telegram_ids` (`withFreshAssignees`, prod 2026-06-16). Деградация (профиль так и без имени) → остаётся «#id», не регресс.

**🔴 Баги**
- **Спринт не создаётся.** Форма «Создать спринт» (`SprintBoard`): даты начала и конца одинаковые (16/06 → 16/06). Расследовать: валидация дат на бэке (`validateTaskDates`/sprint-валидация) или сам POST `/sprints`. Воспроизвести, поймать ответ API.
- **Граф не показывается** (`DependencyGraph`). Пустой/не рендерится. Проверить, есть ли вообще зависимости задач (граф строится по `task_dependencies`, а их, вероятно, нет).
- **miniapp `TaskDetail`: нет описания.** У AI-извлечённых/ручных задач `description=null` (на карточке и в детали пусто). Проверить пайплайн извлечения задач (`extractTasks`/createMeetingTasks) — сохраняется ли `description`; и нужен ли плейсхолдер вместо пустоты.
- **Редактор исполнителя не префиллится** (`TaskModal` показывает «__none__» для задачи с исполнителем). У части задач `assignee_telegram_ids` пуст, а имя есть только в `assignees`. Решить: либо бэкфилл id по имени, либо в модалке добавлять текущее значение в список опций.

**🎨 Редизайн (перевести легаси-виды на дизайн-систему «roy»)**
- **Доска (`KanbanBoard`)**: слишком много пустого пространства, крупные шрифты. Сейчас — одна колонка карточек во всю ширину. Рекомендация: **многоколоночный канбан** (Открыто/В работе/Готово рядом, как уже сделано в `SprintBoard`) + плотные roy-карточки (меньше паддинги, roy-типографика, иконки-действия вместо текстовых кнопок). Референсы плотных бордов: Linear, Height, Trello.
- **Таймлайн (`TimelineView`)**: визуал переработать — занимает много места без явной пользы; **нет разделения по месяцам** (видны только числа 17…7, непонятно где июнь/июль) — добавить заголовки месяцев/границы; «задачи внизу» непонятны — пересмотреть нижний список (что это и зачем).
- **Граф (`DependencyGraph`) — фича:** автоматически собирать темы по странам (кластеризация), а не только ручные зависимости задач. Пересекается с идеей [граф знаний](#веб-интерфейс--граф-знаний) — возможно, объединить: граф знаний/тем с кластерами по странам.
