...
- Вынести дублирующиеся `norm()`/`fmtDate` (SearchScreen ↔ dash/shared) в `lib/utils`.
...
- 🟡 Dark mode — натурный прогон в Telegram WebView на устройстве (код-аудит выполнен, визуал на устройстве не проверялся).
...
- Группировать подряд идущие сообщения / multi-message форварды в одну запись (окно времени или `media_group_id`).
- ✅ **Реализовано (2026-06-19, прод):** дедуп ТОЧНЫХ дублей контента за **7 дней** на уровне воркспейса (`saveEntry`, любой автор) — повторная отправка/вставка и отложенный дубль коллеги не плодят запись; правка контента = не точный матч → сохраняется как вариант. Осталось: near-identical (по сходству, не байт-в-байт) + группировка фрагментов.
- Для «сырого контекста» (форвард/вставка) — более лёгкий путь: сохранять как один note как есть, без агрессивной meeting-классификации; meeting-тип — только для явных транскриптов.
- Подтянуть классификатор страны/типа (или дать ручную правку — частично закрыто экраном «Ревью встреч»: реклассификация/правка/мерж).
