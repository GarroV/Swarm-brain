...
- **Цифры вместо имени исполнителя** («#744230399»). Причина: имя денормализуется в `tasks.assignees` при создании; если профиля ещё не было — туда падал сырой `telegram_id`. Фикс: `swarm-api` `GET /tasks` и `GET /tasks/:id` перерезолвят имена из актуальных `user_profiles` по `assignee_telegram_ids` (`withFreshAssignees`). Деградация (профиль так и без имени) → остаётся «#id», не регресс.
