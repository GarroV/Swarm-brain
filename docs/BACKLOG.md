...
Пользователь прошёлся по всем видам вкладки «Задачи» на десктопе (`TasksScreen`: Доска/Таймлайн/Спринт/Граф — легаси-компоненты на shadcn, **не** переведены на дизайн-систему «roy») и miniapp-детали. Список к разбору:
...
**🟡 Осталось**
- **miniapp/Доска: нет описания у задач.** 57 из 59 задач воркспейса `cee` имеют `description=null` — **пайплайн извлечения не сохраняет описание** (рендер-то есть, данных нет). Разобрать `extractTasks`/`createMeetingTasks`: промпт просит description, но он почти всегда пустой. Либо улучшить промпт, либо это ок (короткие задачи) — решить.
...
- **Таймлайн — «слишком много места без пользы»**: при 6 задачах с датами тело Gantt растянуто на всю высоту (`flex-1`) → много пустой сетки. Косметика/редизайн: ограничить высоту тела контентом, плотнее строки. (Месяцы и нижний блок уже поправлены.)
