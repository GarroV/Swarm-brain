...
#### FEAT-3: Рынки — жёсткий список вместо текстового поля
Сейчас пользователь вводит рынки произвольно (KZ, PL, RS). Нужен фиксированный список.

Что сделать:
1. Определить полный список рынков — **обсудить с командой и прописать в коде**
2. Frontend `SettingsScreen.tsx` (`ProfileSection`): заменить `<Input>` на мультиселект / чекбоксы
3. Рынки могут быть только из списка, но множественный выбор

Варианты реализации (выбрать):
- **Inline chips** (toggle-кнопки) — проще всего для мобайла
- **Выпадающий мультиселект** — компактнее если рынков >10
- **Хардкод в код** vs **таблица в БД** — если список меняется редко, хардкод проще

🟢 Просто (реализация), но сначала нужен список рынков от команды

---

#### FEAT-4: Встречи — редактирование стран
В карточке встречи страны отображаются, но изменить нельзя.
`PATCH /meetings/:id` принимает только `{ confirmed, summary }` — `countries` не поддерживается.

Что сделать:
1. Backend (`swarm-api/index.ts`, `PATCH /meetings/:id`): добавить `countries` в список допустимых полей для обновления
2. Frontend `MeetingsScreen.tsx` (`MeetingDetailDialog`): добавить редактирование стран — chips с крестиком + поле добавления

Вопрос дизайна: тот же жёсткий список что и в профиле, или свободный ввод?
Предложение: для встреч — свободный ввод (страна-контрагент может быть любой), для профиля пользователя — фиксированный список (операционные рынки).
