# Подключение Claude Desktop к Swarm Brain (macOS)

## Самый простой способ — `/setup` (рекомендуется)

1. В боте напиши **`/setup`**.
2. Бот пришлёт **одну команду** с уже вшитым твоим токеном.
3. Открой приложение **Терминал** (⌘+Пробел → набери «Терминал» → Enter), вставь команду (⌘+V), нажми Enter.
4. Скрипт сам всё поставит, пропишет коннектор и перезапустит Claude Desktop. Ничего скачивать не нужно: он кладёт маленький мост на `bash`+`curl` (они есть на любом маке) и правит конфиг штатным `plutil`. Node, npm и интернет к их серверам больше не требуются — только доступ к самому Swarm.

Готово — в Claude появится сервер **swarm-brain** с инструментами. Дальше создай проект и вставь в **Instructions** текст из команды `/claude`.

> Что делает команда: качает скрипт с `…/functions/v1/swarm-setup` и запускает его с твоим токеном (`SWARM_TOKEN=…`). Скрипт безопасно **дописывает** блок `swarm-brain` в `~/Library/Application Support/Claude/claude_desktop_config.json`, не трогая другие серверы (и делает бэкап). Токен в команде — личный и **бессрочный** (живёт, пока не отзовёшь). Отозвать: `/revoketoken`.

### Если что-то пошло не так
- **«command not found» / нет интернета / прокси режет** — скрипт напишет понятную ошибку. Чаще всего достаточно проверить интернет и запустить команду из `/setup` ещё раз.
- **Claude Desktop не установлен** — поставь его, открой один раз, потом запусти команду из `/setup`.
- Команду можно запускать сколько угодно раз — она просто обновит настройку (старый токен при этом перевыпускается).

---

## Ручная настройка (для технических, опционально)

Если не хочешь автоскрипт — можно прописать конфиг руками.

1. В боте: **`/mytoken`** — получишь токен `smcp_…`. Если токен у тебя уже выдан, бот **не перевыпустит его молча** (это сломало бы рабочий config): предупредит и попросит подтвердить кнопкой. Перевыпускай, только если потерял токен или подозреваешь утечку — старый при этом перестаёт работать.
2. Создай/открой `~/Library/Application Support/Claude/claude_desktop_config.json` и добавь блок (не затирая существующие серверы). Замени `<SUPABASE_PROJECT_REF>` на ref своего Supabase-проекта и `smcp_ТВОЙ_ТОКЕН` — на токен из `/mytoken`:

Сначала положи мост (он переводит stdio-поток Claude Desktop в HTTP-запрос к нашему серверу):

```sh
mkdir -p ~/.swarm-brain/bin
curl -fsSL https://<SUPABASE_PROJECT_REF>.supabase.co/functions/v1/swarm-setup \
  | sed -n "/<<'SWARM_BRIDGE_EOF'/,/^SWARM_BRIDGE_EOF$/p" | sed '1d;$d' > ~/.swarm-brain/bin/swarm-mcp-bridge.sh
chmod +x ~/.swarm-brain/bin/swarm-mcp-bridge.sh
```

Затем впиши блок (замени `<ТВОЙ_ЮЗЕР>` на имя своей папки в `/Users`):

```json
{
  "mcpServers": {
    "swarm-brain": {
      "command": "/bin/bash",
      "args": ["/Users/<ТВОЙ_ЮЗЕР>/.swarm-brain/bin/swarm-mcp-bridge.sh"],
      "env": {
        "SWARM_MCP_URL": "https://<SUPABASE_PROJECT_REF>.supabase.co/functions/v1/swarm-mcp",
        "SWARM_MCP_AUTH": "Bearer smcp_ТВОЙ_ТОКЕН"
      }
    }
  }
}
```

> Продакшен Dodo Brands развёрнут на ref `vbqglndbxkpmreccpqmr` — для подключения к нему URL будет `https://vbqglndbxkpmreccpqmr.supabase.co/functions/v1/swarm-mcp`. Свой ref можно найти в дашборде Supabase (Project Settings → General) или в начале URL `…supabase.co`.

3. Пути в `command` и `args` — **абсолютные**: GUI-приложение не наследует PATH шелла, и относительный путь просто не найдётся. Node и `npx` больше не нужны вовсе (было до 2026-08-25, issue #47 — три сетевые точки отказа за корпоративным VPN).
4. Полностью выйди из Claude Desktop (**Cmd+Q**) и открой заново.

> ⚠️ Никогда не используй поле `"url"` / `"type":"http"` в `claude_desktop_config.json` — Claude Desktop его не понимает и **молча стирает весь блок `mcpServers`** (баг anthropics/claude-code#37286). Только форма с `command` + `args`.

> ℹ️ Мост синхронный: запрос → ответ. Серверные события (прогресс, `tools/list_changed`, стриминг) он не поддерживает — `swarm-mcp` их и не использует. Когда понадобятся — issue #94.

---

## Доступные инструменты

| Инструмент | Что делает |
|---|---|
| `add_knowledge` | Добавить текст в базу (или личное хранилище) |
| `upload_file` | Загрузить файл в Storage + создать запись |
| `search_knowledge` | Найти информацию (включая личные записи) |
| `get_entry` | Полный текст записи по ID |
| `get_tasks` | Задачи команды с фильтрами (в строке печатается `id` задачи; у подзадачи — родитель, у родителя — сколько подзадач закрыто) |
| `add_task` / `update_task` с `parent_task_id` | Подзадачи: создать подзадачу или привязать существующую задачу к родителю (вложенность одна, тот же проект); `parent_task_id: ""` в `update_task` — отвязать |
| `get_recent_comments` | Свежие комментарии-апдейты по всем доступным задачам одним запросом (`since`, `limit`) — для дайджеста «что нового» |
| `get_task_stats` | Статистика по задачам за период: сколько создано и закрыто, за сколько дней закрывают, что просрочено, кто сколько закрыл |
| `get_task_history` | История одной задачи — что, когда и кем менялось |
| `get_recent_task_changes` | Все изменения по задачам за период одним запросом — «где, когда, куда передвинули» |
| `get_sprint_spaces` | Пространства раздела «Спринты» (не вкладки доски «Проекты») с живым спринтом — вызывать первым |
| `get_sprints` / `get_sprint` | Список спринтов (по пространству, с принятыми по флагу) / спринт целиком: этап, план и факт, сверка, состав по проектам |
| `create_sprint_space` | Завести пространство спринтов (любой участник); `rename_sprint_space` / `archive_sprint_space` — только админ |
| `create_sprint` / `update_sprint` | Завести спринт в пространстве (имя, старт, финал, чекпоинт) / изменить его, перенести в другое пространство |
| `start_sprint` / `accept_sprint` | Начать спринт (состав становится планом) / принять: итоги, следующий спринт и перенос незакрытого |
| `add_sprint_tasks` / `remove_sprint_task` | Набрать состав по id задач (до старта — в план, после — «сверх плана») / убрать задачу из состава |
| `mark_sprint_task` | Отметка по задаче в спринте: сверка «идёт / риск / проблема» с комментарием, «к переносу» с причиной |
| `delete_sprint` | Убрать непринятый спринт в архив — только админ |
| `get_meetings` | Последние встречи |
| `get_users` | Список команды с профилями |
| `get_storage_stats` | Статистика базы |
| `list_entries` | Список записей с фильтрами (источник, тип, дата, файлы, страны) — для ревизии |
| `delete_entry` | Удалить запись (и прикреплённый файл из Storage) |
| `update_entry` | Обновить запись или заменить файл |
| `reindex_entry` | Перечитать запись и пересчитать страны + embedding через GPT |
