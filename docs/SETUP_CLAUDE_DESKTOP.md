...
| `whoami` | Кто я — от чьего имени работает коннектор (определяется токеном) |
| `get_review_queue` | Мои черновики встреч, ждущие вычитки |
| `get_draft_meeting` / `update_draft_meeting` | Открыть черновик и поправить тезисы или название |
