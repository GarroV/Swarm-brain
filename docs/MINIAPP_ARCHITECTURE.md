...
| `src/components/tasks/TaskQuickActions.tsx` + `QuickPickPopover.tsx` | Быстрые действия в строке: срок (`DatePicker` compact), исполнитель (`fetchUsers`), страна (`COUNTRY_NAMES`). Выбор → `updateTask` PATCH → `r.reload`. `QuickPickPopover` — портал-список (как `DatePicker`: fixed по триггеру, флип, клик-вне, Escape), `stopPropagation` чтобы клик не открыл карточку/не съелся свайпом |
