...
| `src/lib/smartLists.ts` | **Единый источник правды о линзе задач.** Чистая логика смарт-списков в стиле Reminders: `matchesLens` (экспортируется — им же живёт главный экран), `filterTasks`/`countLists`/`groupByMarket`/`groupByAssignee`. Смарт-списки (время/статус): Сегодня·Ближайшие·Все·Готовые. Линза `Lens = mine \| team \| all`: **`mine`** — я в `assignee_telegram_ids`; **`team`** — ОБЩИЕ задачи (`!is_private` И **без конкретного исполнителя**); **`all`** — всё видимое. Личная задача коллеги не попадает НИ в `mine`, НИ в `team` — линзы не делят список пополам. «По рынкам»/«Все сотрудники» — НЕ линзы, а независимые тумблеры (`useReminderTasks`), см. §Модуль задач |
...
| `src/components/tasks/LensToggle.tsx` | Шапка экрана задач: 3-позиционная линза **Мои/Команда/Все** + независимые тумблеры «По рынкам»/«Все сотрудники» (состояние в `localStorage` `roy_tasks_view`, общее для десктопа и мобайла) |
