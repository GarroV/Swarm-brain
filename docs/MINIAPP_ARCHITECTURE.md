...
Три колонки (desktop):
- **Слева (300px):** объединённый список = `fetchAgentMeetings("awaiting_review")` (черновики desktop-agent, первыми) + `fetchMeetings({confirmed:false})` (неподтверждённые встречи Granola/Read.ai). Источники объединены — пользователю это «встречи, требующие решения». Счётчик-фильтр: **админ** — `Segmented` с числами **Все / Личные / Общие** (разбивка по ВЛАДЕЛЬЦУ: `recorders[].telegram_id` для черновика, `owner_id` для entry; админ всегда тянет весь воркспейс `all=true` и делит локально — без перезапроса); **не-админ** — плашка «требуют решения» (видит только своё). (Раньше были два путающих счётчика «на согласовании»/«черновиков», причём первый = всего, второй = подмножество.)
