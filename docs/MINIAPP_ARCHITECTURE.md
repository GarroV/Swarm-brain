...
Три колонки (desktop):
- **Слева (300px):** объединённый список = `fetchAgentMeetings("awaiting_review")` (черновики desktop-agent, первыми) + `fetchMeetings({confirmed:false})` (неподтверждённые Granola/Read.ai). Источники объединены — это «встречи, требующие решения». **Только СВОИ — у всех, включая админа** (own-scoped): чужое непубликованное приватно, в нём не копаемся (`all=true`-ветка убрана). Сверху одна плашка «требуют решения: N». Приглядеть, у кого копится бэклог, админ может **агрегатом без контента** — блок «На вычитке по участникам» в `AdminScreen` (`GET /admin/review-counts`). (Раньше тут были два путающих счётчика «на согласовании»/«черновиков» + админ-режим «Все/Личные/Общие» по всему воркспейсу — убраны как «копание в чужом».)
