...
| `src/lib/smartLists.ts` | Чистая логика смарт-списков в стиле Reminders: `filterTasks`/`countLists`/`groupByMarket`. Смарт-списки (время/статус): Сегодня·Ближайшие·Все·Готовые. Линза (ось «как смотреть»): Мои/Команда/Все/`market` — «market» не фильтр владельца, а группировка активного списка по рынкам (все владельцы) |
...
### Модуль задач (Рой)
Дефолтный вид — **«Список»** в стиле macOS Reminders. Смарт-списки (рельс/чипы, ось время-статус): **Сегодня · Ближайшие · Все · Готовые**. Верхний переключатель-линза (ось «как смотреть»): **Мои · Команда · Все · По рынкам**, где «По рынкам» группирует задачи активного смарт-списка по странам (все владельцы); бинарный чекбокс. (Раньше «Важное» и «По рынкам» были смарт-списками — «Важное» убрано, «По рынкам» переехало в линзу.) Десктоп и мобайл (`RoyTasksScreen`) делят логику через `useReminderTasks` + `TaskRow` + `SmartListNav`; на десктопе смарт-списки — левый рельс, на мобайле — чипы. Виды List / Timeline / Sprint / Projects работают поверх того же `swarm-api` контракта (счётчики и линза считаются на клиенте из общего `fetchTasks`). Канбан остался только в «Спринте». Приватные задачи видны в miniapp только владельцу (фильтрация на бэкенде). PWA устанавливается на macOS: Safari → Поделиться → «Добавить в Dock».
...
| `useDashboardData.ts` | Единый хук данных: параллельно грузит `fetchTasks/Meetings/Entries/AgentMeetings` + **отдельно** очередь на согласовании `fetchMeetings({confirmed:false})` — **ТОЛЬКО свои** (даже у админа): чужое непубликованное приватно, в него не лезем (сводка по участникам — отдельный агрегат `/admin/review-counts` в админ-панели). Берёт `me` из nav. Отдаёт `{loading, mine, team, today, week, noDate, materials, pendingList, recentMeetings, pendingMeetings, reviewCount}`: `pendingList` — свои встречи на согласовании, `recentMeetings` — опубликованные (`confirmed=true`), `pendingMeetings = pendingList.length`. `meId = me?.telegram_id` (если нет — все задачи в `team`, личные секции пусты). `todayISO` — **локальная** дата (`Intl.DateTimeFormat("en-CA")`), т.к. `groupMine` сравнивает как UTC-полночь. Ошибка любого fetch → `[]` (graceful) |
...
| `TeamTasks.tsx` | Право-низ: `team` (незавершённые) — строки `DashTaskRow` с `showAssignee` (аватар исполнителя справа, как на доске). Шапка → `setTab("task")`. Строка → `taskDetail` |
