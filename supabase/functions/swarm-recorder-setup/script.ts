...
say ""
say "Что произойдёт: при необходимости один раз скачаются Command Line Tools, затем"
say "будет создан локальный сертификат подписи (попросит пароль ОДИН раз — это нормально),"
say "приложение соберётся и поставится в /Applications с уже прописанным токеном."
say ""
...
# ── 3. Стабильный сертификат подписи (один запрос пароля) ────────────────────────
...
say "ℹ️  Сейчас может появиться ОДИН системный запрос пароля — это доверие к локальному cert."
if ! ./setup-signing.sh; then
  die "Не удалось создать/доверить сертификат подписи. См. сообщение выше."
fi
