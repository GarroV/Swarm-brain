...
# ── 0. Токен + macOS ────────────────────────────────────────────────────────────
# Токена в команде может не быть — и это НОРМАЛЬНЫЙ путь обновления: если рекордер уже стоит,
# его токен лежит в конфиге, и переустановка не должна требовать перевыпуска. Перевыпуск гасит
# рабочую авторизацию сразу, и брошенное на полпути обновление оставляло человека с рекордером,
# который пишет встречи и молча не может их залить (issue #146).
REUSED_TOKEN=0
if [ -z "\${SWARM_TOKEN:-}" ] && [ -f "$CONFIG" ]; then
  SWARM_TOKEN="$(plutil -extract token raw "$CONFIG" 2>/dev/null || true)"   # plutil читает JSON, он штатный
  [ -n "\${SWARM_TOKEN:-}" ] && REUSED_TOKEN=1
fi
if [ -z "\${SWARM_TOKEN:-}" ]; then
  die "Нет токена, и на этом маке рекордер ещё не настроен.
Возьми команду с токеном в боте: /recordertoken (или в вебе → Настройки → bumblebee)."
fi
case "$SWARM_TOKEN" in
  smcp_*) ;;
...
ARCH="$(uname -m)"
if [ "$REUSED_TOKEN" = "1" ]; then
  say "✓ Беру токен из настроек на этом маке (перевыпуск не нужен) · macOS · arch=$ARCH"
else
...
step "$([ "$REUSED_TOKEN" = "1" ] && echo "Сохраняю прежний токен" || echo "Прописываю токен в конфиг")"
