...
//   • «эта сторона = эта страна» — прошлое ищем по рынку встречи, и рынок берём ТОЛЬКО из
//     явного указания в названии («Dodo Pizza Bulgaria»), без догадок по профилям участников;
...
/**
 * Страна («сторона») созвона — ТОЛЬКО по ЯВНОМУ указанию в названии встречи.
 *
 * Требование владельца 03.09.2026: «это должно быть при явном совпадении. у нас все встречи
 * со странами прям явно прописаны в названии встречи. то есть тут должно быть максимальное
 * совпадение». Поэтому рынки участников из профилей здесь НЕ используются: это догадка, а
 * догадка тянет тезисы чужой страны — хуже, чем не показать ничего.
 *
 * Само распознавание отдано `pickSuggestedMarkets` с одним сигналом `title` — тот же
 * детектор, что подсказывает рынок на вычитке. Своей копии правила нет намеренно: иначе
 * рекордер и веб разойдутся в том, что считают рынком встречи.
 *
 * `null` — в названии страны нет (или их несколько = кросс-маркет).
 */
export function contextCountry(title: string | null): string | null {
