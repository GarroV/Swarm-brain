...
//   • «задачи показываем связанные с этой встречей» — привязанные к той же записи;
...
/** Откуда задача попала в панель: с самой прошлой встречи или просто по этой стране. */
export type TaskSource = "meeting" | "country";

/**
 * Склейка двух выборок задач: сначала привязанные к прошлой встрече («задачи показываем
 * связанные с этой встречей» — владелец), затем добивка по стране до лимита.
 *
 * Добивка нужна по факту данных: живых задач, привязанных к встречам, на проде 22 на 9
 * встречах, и у последней встречи по стране их обычно НЕТ (у BG — ноль). Зато по стране
 * задачи есть: RS 15, SI 13, BG 3. Без добивки секция была бы пустой ровно в том примере,
 * с которого просьба и началась («созвон с Болгарией»).
 *
 * Задачи встречи никогда не вытесняются страновыми: лимит режет хвост, а не голову.
 */
export function mergeContextTasks<T extends { id: string }>(
  meetingTasks: T[],
  countryTasks: T[],
  limit: number,
): Array<T & { source: TaskSource }> {
  const out: Array<T & { source: TaskSource }> = [];
  const seen = new Set<string>();
  for (const t of meetingTasks) {
    if (out.length >= limit) return out;
    if (seen.has(t.id)) continue;
    seen.add(t.id);
    out.push({ ...t, source: "meeting" });
  }
  for (const t of countryTasks) {
    if (out.length >= limit) return out;
    if (seen.has(t.id)) continue;   // та же задача пришла обеими выборками — «встреча» сильнее
    seen.add(t.id);
    out.push({ ...t, source: "country" });
  }
  return out;
}
