...
const task = (
  t: Partial<StatsTask> & { id: string; status: string },
): StatsTask => t;
...
Deno.test("computeTaskStats: без закрытых задач lead time = null, а не 0 (0 соврал бы «мгновенно»)", () => {
  const s = computeTaskStats([
    task({ id: "a", status: "open", created_at: "2026-09-04T00:00:00Z" }),
  ], { sinceISO: SINCE, now: NOW });
  assertEquals([s.leadTimeAvgDays, s.leadTimeMedianDays, s.onTimeRate], [
    null,
    null,
    null,
  ]);
...
Deno.test("computeTaskStats: закрытые по исполнителям, по убыванию; закрытая задача не в openNow", () => {
  ...
  assertEquals(s.closedByAssignee, [{ name: "Вася", count: 2 }, {
    name: "Аня",
    count: 1,
  }]);
...
Deno.test("computeTaskStats: короткое окно раскладывается по ДНЯМ", () => {
  ...
  assertEquals(s.closedByBucket, [{ key: "2026-09-05", count: 2 }, {
    key: "2026-09-07",
    count: 1,
  }]);
  assertEquals(s.createdByBucket, [{ key: "2026-09-03", count: 2 }, {
    key: "2026-09-04",
    count: 1,
  }]);
});
...
Deno.test("computeTaskStats: длинное окно — по МЕСЯЦАМ (иначе 365 строк в выдаче)", () => {
  ...
  assertEquals(s.closedByBucket, [{ key: "2026-07", count: 1 }, {
    key: "2026-08",
    count: 1,
  }, { key: "2026-09", count: 1 }]);
});
...
Deno.test("computeFlowTimes: задачи без перехода в работу в расчёт не идут — basis это показывает", () => {
  ...
  assertEquals([f.basis, f.cycleAvgDays, f.timeToStartAvgDays], [
    0,
    null,
    null,
  ]);
