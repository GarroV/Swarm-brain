...
export function validateParent(input: {
  ...
  if (parentId === projectId) {
    return { ok: false, error: "проект не может быть родителем самому себе" };
  }
  const parent = all.find((p) => p.id === parentId);
  if (!parent) return { ok: false, error: "родитель не найден в воркспейсе" };
  if (parent.parent_id !== null) {
    return { ok: false, error: "нельзя вкладывать глубже 2 уровней" };
  }
  if (projectId !== null && all.some((p) => p.parent_id === projectId)) {
    return {
      ok: false,
      error: "у проекта есть подпроекты — его нельзя делать подпроектом",
    };
  }
  return { ok: true };
}
