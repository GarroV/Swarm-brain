...
//
// К чему привязано: встречные отказы — к meeting_id (встреча из воркспейса агента, её владелец —
// тот, кому уходит сообщение); до-встречные (no_conference_link, no_owner) — к ключу календаря.
//
// Сколько раз: считает СЕРВЕР по журналу `meeting_notices`, а не бот по присланному числу.
// Номер отправки занимается в журнале ДО отправки (уникальный индекс — гонка двух вызовов даёт
// одно сообщение). Дверь — первое уведомление и ровно один повтор, остальные виды — по одному на
// встречу, сверху потолок на встречу и суточные на человека. Исчерпано → 409 и `should_leave: true`.
//
