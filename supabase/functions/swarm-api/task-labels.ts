...
export async function handleTaskLabelRoutes(
  ...
  if (routePath === "/task-labels" && req.method === "POST") {
    ...
    if (!name) {
      return json({ error: "Название метки обязательно" }, 400, origin);
    }
  ...
  const { data: owned } = await supabase
    .from("task_labels").select("id").eq("id", labelId).eq(
      "owner_id",
      telegramId,
    ).maybeSingle();
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await supabase
      .from("task_labels").update(fields).eq("id", labelId).eq(
        "owner_id",
        telegramId,
      )
  ...
  if (req.method === "DELETE") {
    ...
    for (
      const t of (tasksWith ?? []) as Array<{ id: string; label_ids: string[] }>
    ) {
      await supabase.from("tasks").update({
        label_ids: t.label_ids.filter((x) => x !== labelId),
      }).eq("id", t.id);
    }
    await supabase.from("task_labels").delete().eq("id", labelId).eq(
      "owner_id",
      telegramId,
    );
