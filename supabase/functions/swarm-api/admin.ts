...
export async function handleAdminRoutes(
  ...
  if (req.method === "POST" && routePath === "/admin/workspaces") {
    ...
    const id = String(body.id ?? "").trim().toLowerCase().replace(
      /[^a-z0-9-]/g,
      "",
    );
  ...
  if (wsUsersMatch) {
    ...
    if (req.method === "GET") {
      ...
      const { data: profiles } = ids.length
        ? await supabase.from("user_profiles").select(
          "telegram_id, first_name, last_name, role, markets, email, phone, notes",
        ).in("telegram_id", ids)
        : { data: [] };
      ...
      const result = (users ?? [])
        .map((u: Record<string, unknown>) => {
          ...
          return {
            ...
            name: fullName || (u.username ? `@${u.username}` : null) ||
              (u.email as string | null) || (tid != null ? String(tid) : "?"),
            ...
            email: (p?.email as string | null) ?? (u.email as string | null) ??
              null,
            phone: p?.phone ?? null,
            notes: p?.notes ?? null,
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        if (r.status === "workspace_not_found") {
          return apiErr(404, "Workspace not found", origin);
        }
        if (r.status === "bad_input") {
          return apiErr(400, "telegram_id or username required", origin);
        }
        if (r.status === "email_taken") {
          return apiErr(
            409,
            "Этот email уже привязан к другому пользователю",
            origin,
          );
        }
  ...
  const wsUserMatch = routePath.match(
    /^\/admin\/workspaces\/([^/]+)\/users\/([^/]+)$/,
  );
  if (wsUserMatch && req.method === "DELETE") {
    ...
    if (ref.kind === "telegram") {
      if (ref.telegramId === ADMIN_TELEGRAM_ID) {
        return apiErr(400, "Cannot remove super admin", origin);
      }
  ...
  if (wsPatchMatch && req.method === "PATCH") {
    ...
    const { data } = await supabase.from("workspaces").select("*").eq(
      "id",
      wsId,
    ).single();
  ...
  if (userPatchMatch && req.method === "PATCH") {
    ...
    if (ref.kind !== "telegram") {
      ...
      if (extra.length) {
        return apiErr(
          400,
          "До первого входа пользователю можно задать только email",
          origin,
        );
      }
      if (!("email" in body)) {
    ...
    for (
      const k of ["first_name", "last_name", "role", "email", "phone", "notes"]
    ) {
    ...
    if ("email" in body) {
      ...
      if (auErr) {
        if ((auErr as { code?: string }).code === "23505") {
          return apiErr(
            409,
            "Этот email уже привязан к другому пользователю",
            origin,
