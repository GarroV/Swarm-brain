...
function makeSupabase(
  terminal: { data?: unknown; error?: unknown } = { data: { id: 29 } },
): {
...
function patchReq(body: unknown): Request {
  return new Request("https://x/admin", {
    method: "PATCH",
...
Deno.test("PATCH /admin/users/:username — ожидающему приглашению привязывается email (lower)", async () => {
  ...
  const res = await handleAdminRoutes(
    client,
    patchReq({ email: "I.Ravilova@Dodobrands.IO" }),
    "/admin/users/indira56",
    ADMIN,
    true,
    "*",
    noNames,
...
Deno.test("PATCH /admin/users/:email — ожидающее email-only приглашение адресуется по прежней почте", async () => {
  ...
  const res = await handleAdminRoutes(
    client,
    patchReq({ email: "new@dodobrands.io" }),
    "/admin/users/old%40dodobrands.io",
    ADMIN,
    true,
    "*",
    noNames,
...
Deno.test("PATCH — приглашение не найдено → 404", async () => {
  ...
  const res = await handleAdminRoutes(
    client,
    patchReq({ email: "a@b.io" }),
    "/admin/users/nosuchuser",
    ADMIN,
    true,
    "*",
    noNames,
...
Deno.test("PATCH — занятый email → 409 с понятным текстом", async () => {
  ...
  const res = await handleAdminRoutes(
    client,
    patchReq({ email: "taken@dodobrands.io" }),
    "/admin/users/indira56",
    ADMIN,
    true,
    "*",
...
Deno.test("PATCH реального юзера по telegram_id — прежний путь профиля не тронут", async () => {
  ...
  const upsert = calls.find((c) => c.method === "upsert")!.args[0] as Record<
    string,
    unknown
  >;
...
Deno.test("DELETE ожидающего по username — только строки без telegram_id и внутри воркспейса", async () => {
  ...
  const res = await handleAdminRoutes(
    client,
    req,
    "/admin/workspaces/cee/users/indira56",
    ADMIN,
    true,
    "*",
    noNames,
  ...
  assertEquals(calls.filter((c) => c.method === "eq").map((c) => c.args), [[
    "group_id",
    "cee",
  ], ["username", "indira56"]]);
});
...
Deno.test("DELETE суперадмина запрещён", async () => {
  ...
  const res = await handleAdminRoutes(
    client,
    req,
    `/admin/workspaces/cee/users/${ADMIN}`,
    ADMIN,
    true,
    "*",
    noNames,
