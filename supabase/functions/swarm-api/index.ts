...
import { parseLinks } from "../_shared/tasks/links.ts";
import {
  COUNTRY_NAMES,
  detectQueryCountry,
  normalizeCountries,
...
async function withEntries(
  ...
  try {
    ...
    if (e instanceof EntryAccessError) {
...
async function withImporterNames<
  T extends { id?: string; owner_id?: number | null; metadata?: unknown },
>(
  rows: T[],
  fallbackById?: Map<string, number>,
): Promise<Array<T & { importer_name: string | null }>> {
  const idOf = (r: T): number | null => {
    const raw = (r.metadata as Record<string, unknown> | null)
  ...
  const names = await resolveNames([
    ...new Set(rows.map(idOf).filter((n): n is number => n !== null)),
  ]);
  return rows.map((r) => {
    const id = idOf(r);
    return {
      ...r,
      importer_name: id !== null ? (names.get(id) ?? null) : null,
    };
  });
}
...
async function resolveAssignee(
  ...
  const { data: au } = await supabase
    .from("allowed_users").select("telegram_id").eq("telegram_id", telegramId)
    .maybeSingle();
  if (!au) return null; // не член воркспейса — не назначаем
  const name = (await resolveNames([telegramId])).get(telegramId) ??
    String(telegramId);
...
>(tasks: T[]): Promise<T[]> {
  ...
  return tasks.map((t) => {
    ...
    const fresh = tids.map((id) => nameById.get(id)).filter((n): n is string =>
      !!n
    );
...
async function streamExtractTasks(
  text: string,
  origin: string,
): Promise<Response> {
  ...
  if (!upstream.ok || !upstream.body) {
    return apiErr(502, "Extractor unavailable", origin);
  }
    ...
    async start(controller) {
      ...
      try {
        outer: while (true) {
          ...
          const { deltas, rest, done: upstreamDone } = readOpenAiSseChunk(
            sseTail,
            false,
          );
          sseTail = rest;
...
function deriveEntryTitle(
  ...
  const metaTitle = typeof e.metadata?.title === "string"
    ? (e.metadata.title as string).trim()
    : "";
...
function entryTagKey(
  entryType: string,
  metadata: Record<string, unknown>,
): string {
  const ft = typeof metadata?.file_type === "string"
...
function validateTaskDates(
  ...
  if (start && due && start > due) {
    return "start_date не может быть позже due_date";
  }
  return null;
}

// Проверяет, что спринт существует и принадлежит тому же воркспейсу.
async function sprintInWorkspace(
  sprintId: string,
  groupId: string,
): Promise<boolean> {
  const { data } = await supabase
    .from("sprints").select("id").eq("id", sprintId).eq("group_id", groupId)
    .maybeSingle();
  return !!data;
}
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "GET" && routePath === "/recorder/setup") {
    ...
    return json(
      { ...st, updateOneLiner: buildRecorderUpdateOneLiner() },
      200,
      origin,
    );
  }
  ...
  if (req.method === "POST" && routePath === "/recorder/token") {
    ...
    return json(
      {
        oneLiner: buildRecorderSetupOneLiner(minted.token),
  ...
  if (req.method === "GET" && routePath === "/mcp/instructions") {
    return json(
      { instructions: buildClaudeProjectPrompt(telegram_id) },
      200,
      origin,
    );
  }
  ...
  const commentResp = await handleTaskCommentRoutes(
    supabase,
    req,
    routePath,
    telegram_id,
  ...
  const cycleResp = await handleSprintCycleRoutes(
    req,
    routePath,
    telegram_id,
    groupId,
  ...
  // Журнал пространства (/spaces/:tabId/journal) — лента событий вкладки. Тоже модулем.
  ...
  if (routePath === "/tasks") {
    if (req.method === "GET") {
      ...
      const limit = parseListLimit(url.searchParams.get("limit"), {
        def: TASKS_LIST_LIMIT,
        max: TASKS_LIST_LIMIT,
      });
      const confirmedParam = url.searchParams.get("confirmed");
      const confirmedFilter = confirmedParam === "true"
        ? true
      ...
      if (tasks.length >= limit) {
        console.warn(
          `[tasks] список упёрся в лимит ${limit} — часть задач не попала в ответ (issue #111/#112)`,
        );
      }
      ...
      const tasksWithCreator = (await withFreshAssignees(tasks)).map((t) => ({
        ...t,
        created_by_name: t.created_by_telegram_id != null
          ? (creatorMap.get(t.created_by_telegram_id) ?? null)
          : null,
      }));
    ...
    if (req.method === "POST") {
      ...
      if (safeMeetingId) {
        try {
          await getEntrySecure(supabase, safeMeetingId, {
            groupId,
      ...
      // Ссылки рисуются как <a href>, поэтому схема проверяется явным списком: `javascript:`
      // — это выполнение чужого кода, а не адрес. Отказ с номером негодной ссылки.
      let links;
      try {
        links = parseLinks(body.links);
      } catch (e) {
        return apiErr(400, e instanceof Error ? e.message : String(e), origin);
      }
      ...
      const input: TaskInput = {
        ...
        priority: ["high", "med", "low"].includes(body.priority as string)
        ...
        timeline_position: typeof body.timeline_position === "number"
  ...
  if (taskMatch) {
    ...
    if (req.method === "PATCH") {
      ...
      if (body.description !== undefined) {
        fields.description = body.description as string | null;
      }
      if ("links" in body) {
        try {
          fields.links = parseLinks(body.links);
        } catch (e) {
          return apiErr(
            400,
      ...
      if (body.country !== undefined) {
        fields.country = body.country as string | null;
      }
      if (body.task_role !== undefined) {
        fields.task_role = body.task_role as string | null;
      }
      ...
      if ("start_date" in body) {
        fields.start_date = body.start_date as string | null;
      }
      if (typeof body.timeline_position === "number") {
        fields.timeline_position = body.timeline_position;
      }
      if (Array.isArray(body.tags)) fields.tags = body.tags as string[];
      if (body.priority !== undefined) {
        fields.priority =
          ["high", "med", "low"].includes(body.priority as string)
            ? (body.priority as string)
            : null;
      }
      ...
      if (typeof body.project_linked === "boolean") {
        const effProject = "project_id" in fields
          ? fields.project_id
          : task.project_id;
      ...
      if ("parent_id" in body) {
        ...
        if (rawParent) {
          ...
          const { data: proj } = await (task.project_id
            ? sibQ.eq("project_id", task.project_id)
            : sibQ.is("project_id", null));
          const parents = new Map(
            ((proj ?? []) as Array<{ id: string; parent_id: string | null }>)
          ...
          while (cur && guard++ < 1000) {
            if (cur === taskId) {
              return apiErr(
                400,
                "нельзя привязать к своему потомку (цикл)",
                origin,
              );
            }
      ...
      const effStart = "start_date" in fields
        ? fields.start_date
        : task.start_date;
      ...
      if ("recur_freq" in body) {
        ...
        if ("recur_anchor_dom" in recur) {
          fields.recur_anchor_dom = recur.recur_anchor_dom;
        }
      }
      ...
      const effFreq = "recur_freq" in fields
        ? fields.recur_freq
        : task.recur_freq;
      if (effFreq && !effDue) {
        return apiErr(
          400,
          "У регулярной задачи должен быть срок: задай срок или сними цикличность",
          origin,
        );
      }
      ...
      try {
        ...
        if (fields.project_linked === false) {
          ...
          ((proj ?? []) as Array<{ id: string; parent_id: string | null }>)
            .forEach((t) => {
              if (t.parent_id) {
                const a = kids.get(t.parent_id) ?? [];
                a.push(t.id);
                kids.set(t.parent_id, a);
              }
            });
          ...
          if (subtree.length) {
            ...
            }).in("id", subtree).eq("group_id", groupId);
          }
        }
  ...
  if (routePath === "/sprints") {
    ...
    if (req.method === "POST") {
      ...
      if (!body.name || typeof body.name !== "string") {
        return apiErr(400, "name is required", origin);
      }
      if (!body.start_date || !body.end_date) {
        return apiErr(400, "start_date и end_date обязательны", origin);
      }
  ...
  if (sprintMatch) {
    ...
    if (req.method === "PATCH") {
      ...
      if (typeof body.status === "string") {
        fields.status = body.status as SprintInput["status"];
      }
  ...
  if (sprintTasksMatch) {
    ...
    const taskIds = Array.isArray(body.task_ids)
      ...
      : [];
    if (req.method === "POST") {
      if (!(await sprintInWorkspace(sprintId, groupId))) {
        return apiErr(404, "Not found", origin);
      }
  ...
  if (routePath === "/projects") {
    ...
    if (req.method === "POST") {
      ...
      const extra = parseProjectFields(body);
      if (extra.error) return apiErr(400, extra.error, origin);
      ...
      try {
        return json(
          await createProject(input, groupId, telegram_id ?? null),
          201,
          origin,
        );
      } catch (e) {
  ...
  if (projectMatch) {
    ...
    if (req.method === "PATCH") {
      ...
      // Сроки сверяются с тем, что уже стоит у инициативы: в теле обычно приходит один конец,
      // и без текущего значения «перенести конец раньше старта» прошло бы молча.
      const current = await getProject(projectId, groupId);
      const extra = parseProjectFields(body, current ?? {});
      if (extra.error) return apiErr(400, extra.error, origin);

      const fields: Partial<ProjectInput> = { ...extra.fields };
      if (typeof body.name === "string") fields.name = body.name.trim();
      if ("color" in body) fields.color = body.color as string | null;
      if ("emoji" in body) fields.emoji = body.emoji as string | null;
      if ("parent_id" in body) {
        fields.parent_id = (body.parent_id as string | null) ?? null;
      }
      if (typeof body.is_private === "boolean") {
        fields.is_private = body.is_private;
      }
  ...
  if (req.method === "GET" && routePath === "/entries") {
    ...
    let q = buildEntriesQuery(supabase, ENTRY_COLUMNS, {
      groupId,
      ...
      .limit(
        parseListLimit(url.searchParams.get("limit"), { def: 500, max: 2000 }),
      );
  ...
  if (req.method === "POST" && routePath === "/entries/upload") {
    ...
    try {
      ...
      return apiErr(400, "Invalid form data", origin);
    }
    ...
    const addedBy = (profile as { first_name?: string } | null)?.first_name ||
      String(telegram_id);

    const { data: entry, error: insertError } = await supabase.from("entries")
      .insert({
  ...
  if (req.method === "POST" && routePath === "/entries") {
    ...
    if (!body.content || typeof body.content !== "string") {
      return apiErr(400, "content required", origin);
    }
    const isPrivate = body.is_private === true;
    ...
    const { data: entry, error } = await supabase.from("entries").insert({
      ...
      entry_type: meta.entry_type === "meeting" ? "meeting" : "note",
  ...
  if (req.method === "POST" && routePath === "/ask") {
    ...
    try {
      matched = await matchEntries(supabase, embedding, {
        groupId,
        requestingUserId: telegram_id,
        limit: 8,
        queryText: q,
        country: detectQueryCountry(q),
    ...
    const sources = matched.map((e, i) => ({
      ...
      snippet: (e.summary || e.content || "").replace(/\s+/g, " ").trim().slice(
        0,
        220,
      ),
      ...
      return json(
        {
          query: q,
          answer:
            "По базе, встречам и задачам ничего релевантного не нашлось. Попробуй переформулировать запрос.",
          sources: [],
          followups: [],
        },
        200,
    ...
    const ctx = sources.map((s) =>
      `[${s.n}] (${s.entry_type}${s.market ? ", " + s.market : ""}${
        s.date ? ", " + s.date : ""
      }) ${s.title} — ${s.snippet}`
    ).join("\n");
    ...
    if (!askRes.ok) {
      ...
      return json(
        { query: q, answer: "", sources, followups: [] },
        200,
        origin,
      );
    }
    ...
    try {
      ...
      followups = Array.isArray(parsed.followups)
        ? parsed.followups.filter((x: unknown) => typeof x === "string").slice(
          0,
          3,
        )
        : [];
  ...
  if (req.method === "GET" && routePath === "/meetings") {
    ...
    if (confirmedParam === "true") {
      q = q.eq("metadata->>confirmed", "true");
    }
    ...
    if (rows.length) {
      ...
      for (
        const m of (mrows ?? []) as Array<
          {
            entry_id: string | null;
            recorders: Array<{ telegram_id?: number | string }> | null;
          }
        >
      ) {
    ...
    return json(
      isReviewQueue ? named : named.map(toListRow),
      200,
      origin,
      count != null ? { "X-Total-Count": String(count) } : undefined,
    );
  ...
  const meetingResummarizeMatch = routePath.match(
    /^\/meetings\/([^/]+)\/resummarize$/,
  );
  if (meetingResummarizeMatch && req.method === "POST") {
    return withEntries(origin, async () => {
      ...
      if (!meetingRowId) {
        return apiErr(
          400,
          "У записи нет транскрипта встречи — переобработка недоступна",
          origin,
        );
      }
  ...
  if (meetingNotesMatch && req.method === "GET") {
    return withEntries(origin, async () => {
      const entry = await getEntrySecure(supabase, meetingNotesMatch[1], {
        groupId,
        telegramId: telegram_id,
      });
      ...
      const { data: versions } = await supabase.from("meetings")
        .select(
          "id, claim_owner, source, identity_kind, started_at, recorded_seconds",
        )
        .eq("group_id", groupId)
        .eq("entry_id", entry.id);
      const ids = new Set<string>(
        ((versions ?? []) as Array<{ id: string }>).map((v) => v.id),
      );
      const ownMeetingId = (entry.metadata as { meeting_id?: string } | null)
        ?.meeting_id;
      if (ownMeetingId) ids.add(ownMeetingId);
      if (ids.size === 0) {
        return json({ live: [], personal: [], versions: 0 }, 200, origin);
      }
      const idList = [...ids];
      ...
      const names = await resolveNames([
        ...new Set(
          liveRows.map((r) => r.author_id).filter((n): n is number =>
            typeof n === "number"
          ),
        ),
      ]);
          ...
          live: liveRows.map((r) => {
            ...
            return {
              ...
              author_name: row.author_id != null
                ? (names.get(row.author_id) ?? `#${row.author_id}`)
                : null,
            };
          }),
          personal: ((personal ?? []) as Array<
  ...
  if (meetingMatch) {
    ...
    return withEntries(origin, async () => {
      ...
      if (req.method === "PATCH") {
        ...
        if ("entry_type" in body && typeof body.entry_type === "string") {
  ...
  const agentPublishMatch = routePath.match(
    /^\/agent-meetings\/([^/]+)\/publish$/,
  );
  const agentNotesMatch = routePath.match(/^\/agent-meetings\/([^/]+)\/notes$/);
  const agentResummarizeMatch = routePath.match(
    /^\/agent-meetings\/([^/]+)\/resummarize$/,
  );
  ...
  if (
    ...
    if (
      !canAccessDraftMeeting(
        meeting as DraftMeetingRow | null,
        telegram_id,
        isAdmin,
        groupId,
      )
    ...
    if (agentResummarizeMatch && req.method === "POST") {
      if (meeting.status === "in_base") {
        return apiErr(409, "Уже опубликовано — правьте запись в базе", origin);
      }
      let note = "";
      try {
        const b = await req.json() as { note?: unknown };
        note = typeof b?.note === "string" ? b.note.slice(0, 500) : "";
      } catch { /* тело необязательно */ }
      await resummarizeFromTranscript(supabase, mId, note);
      const { data } = await supabase.from("meetings").select("*").eq("id", mId)
        .single();
      const [enriched] = await withRecorderNames([
        (data ?? {}) as { recorders?: unknown },
      ]);
    ...
    if (agentMarketMatch && req.method === "GET") {
      ...
      if (emails.size > 0) {
        ...
        const ids = ((users ?? []) as Array<
          { telegram_id: number | null; email: string | null }
        >)
          .filter((u) =>
            u.telegram_id && emails.has((u.email ?? "").trim().toLowerCase())
          )
          .map((u) => u.telegram_id as number);
        if (ids.length > 0) {
          const { data: profiles } = await supabase.from("user_profiles")
            .select("markets").in("telegram_id", ids);
          participantMarkets =
            ((profiles ?? []) as Array<{ markets: string[] | null }>)
              .map((pr) => pr.markets ?? []).filter((m) => m.length > 0);
        }
      }
      let suggestion = pickSuggestedMarkets({
        title,
        participantMarkets,
        notesMarkets: [],
      });
      ...
      if (!suggestion.source && draftNotes) {
        try {
          const meta = await extractEntryMeta(
            draftNotes,
            Deno.env.get("OPENAI_API_KEY")!,
          );
          suggestion = pickSuggestedMarkets({
            title,
            participantMarkets,
            notesMarkets: meta.countries,
          });
        } catch (e) {
          ...
          console.error(
            "market-suggestion: классификатор по тезисам не ответил",
            e,
          );
        }
      }
    ...
    if (agentMeetingMatch && req.method === "PATCH") {
      ...
      if (Object.keys(upd).length === 1) {
        return apiErr(400, "Нужно draft_notes_md или title", origin);
      }
  ...
  if (req.method === "GET" && routePath === "/calendar/today") {
    ...
    const { data: rec, error: recErr } = await supabase
      .from("allowed_users")
      .select(
        "recorder_last_seen, recorder_last_recording, recorder_last_on_call, recorder_last_meeting_key",
      )
      .eq("telegram_id", telegram_id).maybeSingle();
    if (recErr) {
      console.warn(
        "[calendar/today] присутствие рекордера не прочиталось:",
        recErr.message,
      );
    }
    const r = rec as {
      ...
      recorder_last_on_call?: boolean | null;
      recorder_last_meeting_key?: string | null;
    } | null;
    const presence: RecorderPresence | null = r
      ? {
        meetingKey: r.recorder_last_meeting_key ?? null,
        onCall: r.recorder_last_on_call === true,
        recording: r.recorder_last_recording === true,
        lastSeen: r.recorder_last_seen ?? null,
      }
      : null;

    return json(
      { meetings: todayMeetings(items, new Date(), joinLink, presence) },
      200,
      origin,
    );
  }
  ...
  if (req.method === "GET" && routePath === "/granola/notes") {
    const { data: integration } = await supabase.from("user_integrations")
      .select("api_key, skipped_note_ids").eq("telegram_id", telegram_id).eq(
        "service",
        "granola",
      ).maybeSingle();
    ...
    const importedIds = new Set(
      (imported ?? []).map((e: { metadata: Record<string, unknown> }) =>
        e.metadata?.granola_note_id as string
      ).filter(Boolean),
    );

    const unprocessed = (notes ?? []).filter((n) =>
      !skipped.includes(n.id) && !importedIds.has(n.id)
    );
    return json(unprocessed, 200, origin);
  }

  // ── GET /granola/notes/:id/preview ────────────────────────────────────────────
  const granolaPreviewMatch = routePath.match(
    /^\/granola\/notes\/([^/]+)\/preview$/,
  );
  if (granolaPreviewMatch && req.method === "GET") {
    const noteId = granolaPreviewMatch[1];
    const { data: integration } = await supabase.from("user_integrations")
      .select("api_key").eq("telegram_id", telegram_id).eq("service", "granola")
      .maybeSingle();
    if (!integration) return apiErr(404, "Granola not connected", origin);
  ...
  const granolaImportMatch = routePath.match(
    /^\/granola\/notes\/([^/]+)\/import$/,
  );
  if (granolaImportMatch && req.method === "POST") {
    ...
    const ts = calEvent?.scheduled_start_time as string | undefined ??
    ...
    const date = ts
      ...
      : "";
    const attendees = (note.attendees as
      | Array<{ name?: string; email?: string }>
      | undefined) ?? [];
    const attendeeNames = attendees.map((a) => a.name || a.email || "").filter(
      Boolean,
    ).join(", ");
    ...
    if (dup) {
      ...
      return json(
        { duplicate: true, id: dup.id, title: dup.title },
        200,
        origin,
      );
    }
  ...
  if (granolaSkipMatch && req.method === "POST") {
    ...
    const { data: integration } = await supabase.from("user_integrations")
      .select("skipped_note_ids").eq("telegram_id", telegram_id).eq(
        "service",
        "granola",
      ).maybeSingle();
    if (!integration) return apiErr(404, "Granola not connected", origin);
  ...
  if (req.method === "POST" && routePath === "/feedback") {
    ...
    if (contentType.includes("application/json")) {
      ...
      if (typeof body.text !== "string" || !body.text.trim()) {
        return apiErr(400, "text required", origin);
      }
      text = body.text.trim();
  ...
  if (req.method === "POST" && routePath === "/digest") {
    ...
    const { data: profile } = await supabase.from("user_profiles")
      .select("first_name, last_name, role, markets").eq(
        "telegram_id",
        telegram_id,
      ).maybeSingle();
    const p = profile as {
    ...
    const countryVariants = [
      ...new Set(markets.flatMap((m) => {
        const code = COUNTRY_NAMES[m]
          ? m
          : (NAME_TO_CODE[m.toLowerCase()] ?? m);
    ...
    const meaningfulCountries = (cs?: string[]): string[] =>
      (cs ?? []).filter((c) => c !== "General");
    ...
    const marketCodes = new Set(
      markets.map((
        m,
      ) => (COUNTRY_NAMES[m] ? m : (NAME_TO_CODE[m.toLowerCase()] ?? m))),
    );
    ...
    const entriesText = (entries as EntryRow[]).map((e, i) => {
      ...
      return `[источник ${i + 1}] [${
        entryCountryLabel(e.countries)
      } · ${date}] ${(e.summary ?? e.content).slice(0, 500)}`;
    }).join("\n\n---\n\n");
