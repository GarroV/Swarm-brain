...
export async function handleTaskCommentRoutes(
  ...
  if (listMatch && req.method === "GET") {
    ...
    const names = await resolveNames(
      rows.map((r) => r.added_by_telegram_id).filter((x): x is number => !!x),
    );
    return json(
      rows.map((r) => ({
        id: r.id,
        ...
        author_name: r.added_by_telegram_id
          ? (names.get(r.added_by_telegram_id) ??
            String(r.added_by_telegram_id))
          : "—",
  ...
  if (oneMatch && req.method === "DELETE") {
    ...
    const { data: c } = await supabase
      .from("task_comments").select("id, added_by_telegram_id").eq(
        "id",
        commentId,
