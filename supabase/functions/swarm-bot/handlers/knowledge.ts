  ...
  const messages: Record<string, unknown>[] = [
    {
      ...
      content:
        ...
        "4. Для 'дай/найди/покажи запись X', 'есть ли запись о X', 'отмени/удали запись X' → search_knowledge по названию или теме. НИКОГДА не используй для этого list_personal.\n" +
        "5. Для любого другого вопроса → search_knowledge.\n" +
        "\nХРАНИЛИЩА И ВИДИМОСТЬ (важно для формулировок):\n" +
        "- search_knowledge / find_link / find_note ищут по ВСЕЙ доступной базе: общая база команды + личные записи пользователя. Это НЕ «личное хранилище».\n" +
        "- list_personal — отдельный инструмент только для приватных записей пользователя. Вызывай его ТОЛЬКО при явной просьбе про личное ('моё личное', 'что я сохранял лично', 'мои приватные записи').\n" +
        "- Если искал через search_knowledge/find_link/find_note и не нашёл — пиши «в базе знаний не нашёл». НИКОГДА не пиши «в вашем личном хранилище» — это вводит в заблуждение, потому что искал ты по всей базе.\n" +
        ...
        "- Если найдена инструкция или справочная статья (как подключить, как использовать, шаги настройки) — выводи её текст ЦЕЛИКОМ, без пересказа и сокращений\n" +
