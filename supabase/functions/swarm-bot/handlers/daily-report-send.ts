import { draftMeetingsOwnScopedFilter } from "../../_shared/meeting-access.ts";
...
export async function sendDailyReport(): Promise<void> {
  ...
  // его черновики (записывал или совладелец, `draftMeetingsOwnScopedFilter`), а не ВСЕ черновики глобально — иначе
  // «На вычитке: N» расходится с пустой «Доской встреч» (админ-байпаса для встреч нет; issue #20).
  // Фильтр общий с /agent-meetings.
  const loadReviewCount = async (): Promise<number> => {
    const { count, error } = await supabase
      .from("meetings")
      .select("id", { count: "exact", head: true })
      .eq("status", "awaiting_review")
      .or(draftMeetingsOwnScopedFilter(ADMIN_USER_ID));
    if (error) throw new Error(error.message);
    return count ?? 0;
  };
