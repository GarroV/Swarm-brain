  ...
  try {
    ...
    if (command === "/reset") {
      ...
      await sendMessage(chatId,
        ...
        `🌐 <b>Приложение «Рой»</b> — задачи, встречи, поиск по базе:\n` +
        `🔗 https://swarm-brain.pages.dev — вход через Telegram.\n` +
        `Можно установить как приложение: на ноуте — «Установить» в браузере (встанет в Dock), на телефоне — «На экран Домой».\n\n` +
        `<b>Записывать встречи (Mac):</b>\n` +
        `🎙 <b>Рекордер</b> — пишет звук созвона, сервер делает расшифровку и тезисы → встреча попадает в /meetings.\n` +
        `Установка одной командой: /recordertoken\n\n` +
