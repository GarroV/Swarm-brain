-- Поиск: окно свежести + ступенчатый буст recency в match_entries_hybrid.
-- Мотив (владелец): на «какие новости по болгарии» июльская встреча (вчерашняя) уходила под #7,
-- июнь/май наверху — прежний тайбрейк по свежести был слишком слаб. Нужно: сначала окно ≤2 недель;
-- если там <5 источников — добираем старые; и чем свежее источник, тем больше веса.
--
-- ВНИМАНИЕ: добавляются 2 параметра (fresh_days, min_fresh) → CREATE OR REPLACE создаёт НОВУЮ
-- перегрузку, а не заменяет старую. Поэтому СНАЧАЛА дропаем старую 12-параметровую сигнатуру,
-- иначе вызов по общим именованным аргументам становится неоднозначным и ломает /search и /ask.

drop function if exists public.match_entries_hybrid(
  text, text, integer, bigint, text, text, text,
