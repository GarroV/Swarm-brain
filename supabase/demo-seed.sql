...
insert into entries (content, summary, added_by, source, entry_type, entry_date, metadata, countries, group_id, is_private) values
  ('Синк по маркетингу — Сербия. Запуск летнего меню и промо-кампания.',
   E'### Летнее меню\n- Старт на следующей неделе, 12 новых позиций.\n- Промо-баннеры готовы, отправлены в печать.\n### Бюджет\n- Таргет на запуск увеличен на 15%.',
   'granola', 'granola', 'meeting', (now() - interval '3 days')::date,
   jsonb_build_object('title','Маркетинг — Сербия','confirmed',true), array['RS'], 'demo', false),
  ('Операционка Болгарии. Поставки и открытие новой точки.',
   E'### Поставки\n- Договор с поставщиком упаковки продлён на год.\n- Логистика по региону оптимизирована.\n### Новая точка\n- Помещение согласовано, ремонт стартует в июле.',
   'granola', 'granola', 'meeting', (now() - interval '2 days')::date,
   jsonb_build_object('title','Операции — Болгария','confirmed',true), array['BG'], 'demo', false),
  ('Стенограмма продуктового ревью.',
   E'### Продукт\n- Интеграция платежей в работе, релиз через неделю.\n- Анализ оттока: строим когорты по каналам привлечения.\n### Риски\n- Провайдер платежей задерживает документы.',
